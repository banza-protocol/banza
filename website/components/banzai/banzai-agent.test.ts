...
describe("BanzAI single-shell naming + navigation (M2.19EF2)", () => {
  ...
  it("exposes the three modes (Modos): Perguntar · Validar operador · Onboarding (M2.19G.1/ADR-068, M2.19G.3/ADR-069)", () => {
  ...
  it("collapses Resultados to ONE entry and Recursos to Guia · Referência · Programadores (ADR-068 §29)", () => {
...
describe("authority boundary copy (BX0)", () => {
  ...
  it("suggests operational duration/metric demonstrators (ADR-078)", () => {
...
describe("developer commands are Rust-first (BX0; SimB retired from active surfaces — ADR-074)", () => {
  it("uses banza-conformance-rs / banza-trust, not the outdated pip install, and no longer advertises SimB", () => {
    ...
    // ADR-074: SimB is retired from active surfaces. The banza-simb engine, its SIMB_PRE_REVIEW gate
