// BanzAI validation mode — the endpoint-originated 9-step operator-validation journey (M2.19G.1, ADR-068).
//
...
// pre-computed result or manually chosen artifact EVER enters this path (ADR-068 §4.4/§4.5 — those live
...
// ---------------------------------------------------------------------------------------------------
// Step catalogue (ADR-068 §21 — the canonical nine-step spine). Each step names the Rust engine that
...
export const STEPS: StepMeta[] = [
  ...
  },
  {
    ...
    engine: "banza-target-registry",
    blurb:
      "Agrega em Rust os veredictos das oito etapas técnicas numa Prontidão de Certificação (READY/BLOCKED). Não emite um Registo de Certificação e nunca devolve CERTIFIED (ADR-068 §4.10).",
  },
];
...
export const STATUS_LABEL_PT: Record<StepStatus, string> = {
  ...
  FAILED: "Falhou",
  BLOCKED: "Bloqueado",
  // ADR-077: a step out of scope for the declared profile — never a failure.
...
function stepStatusOf(receipt: ServerOperationReceipt): StepStatus {
  // ADR-077: a NOT_APPLICABLE step is sealed with result.status="NOT_EVALUATED" + applicability
...
// ---------------------------------------------------------------------------------------------------
// Progress language (ADR-068 §28) — "steps evaluated" is distinct from the certification outcome.
// ---------------------------------------------------------------------------------------------------
...
export interface ValidationSession {
  ...
  /** M2.19G.4 (ADR-070) — atomically seed operator + implementation from a route segment. Both are
  ...
  /** ADR-076 correction 1 — the honest durable-persistence verdict of the last full-journey run (null
  ...
  /** ADR-076 D-076-08 — re-check whether a not-yet-persisted run has since become durable. Reads the
...
export function useValidationSession(opts: {
  ...
  // plus an AbortController for the in-flight fetch (safe cancel — ADR-068 §24 running state).
  ...
  const selectOperator = useCallback(
    (id: string) => {
      ...
      abortInFlight();
      setOperatorId(id);
      // Reset the implementation: it must be re-chosen for the new operator (ADR-068 §12).
  ...
  // M2.19G.4 (ADR-070) — atomic route-seed. Sets operator + implementation in ONE update so a deep link
  ...
  // ADR-076 D-076-08 — re-check durability of a run whose persistence was not confirmed. Reads the
  ...
  const progress = useMemo<ProgressCounts>(() => {
    ...
    return {
      ...
      // ADR-077: NOT_APPLICABLE steps are out of scope — not counted as "evaluated", tracked separately.
