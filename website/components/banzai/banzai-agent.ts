...
// M2.19G.1 (ADR-068) — the shell now has a SINGLE "Resultados" area (in-area sub-views, not separate
...
/** The modes of the single BanzAI shell (the "Modos" sidebar group). M2.19G.3 (ADR-069) adds
...
export const BANZAI_AGENT = {
  name: "BanzAI",
  // M2.14I (ADR-054) — BanzAI is the primary human-operator interface of the protocol.
  ...
  boundary:
    "BanzAI é a interface primária de trabalho entre humanos/operadores e o protocolo BANZA (ADR-054); em concreto, o agente do protocolo. Guia, simula, invoca ferramentas verificáveis e explica resultados com base em fontes oficiais — não aprova, não certifica, não licencia, não publica operadores, não movimenta fundos, não decide participação, não inventa regras e não cria decisões arquitecturais. A conformidade demonstra-se por evidência verificável.",
...
export const MODES: { mode: WbMode; icon: WbIcon; name: string }[] = [
  ...
  // M2.19G.1 (ADR-068 §4.1) — the human-facing feature is "Validar operador" (the technical object
...
// Programadores (ADR-068 § Recursos), not in the primary nav.
...
// M2.19G.1 (ADR-068 §11) — the endpoint-originated validation terminology + required intro/result copy.
...
// M2.19G.3 (ADR-069) — BanzAI-hosted operator onboarding copy. Onboarding is a hosted BanzAI service,
...
// M2.19G.1 (ADR-068 §17) — the developer draft tool copy. A draft result is LOCAL, non-authoritative,
...
// Task-oriented suggestions — broad operational duration/metric demonstrators (ADR-078), not
...
export const RFC_DOCS = [
  ...
  { id: "ADR-054", title: "BanzAI — interface primária", q: "Explica o ADR-054 — o BanzAI como interface primária humano-operador" },
  { id: "ADR-021", title: "Evidência e conformidade", q: "Explica o ADR-021 — evidência e conformidade" },
  { id: "ADR-038", title: "Confiança e chaves", q: "Explica o ADR-038 — confiança e chaves" },
  { id: "ADR-037", title: "Rust-first para engines oficiais", q: "Explica o ADR-037 — a política Rust-first" },
...
// Programadores — Rust-first commands (no outdated pip install; ADR-037).
