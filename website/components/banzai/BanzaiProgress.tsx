...
import {
  ...
  realizeProgressLine,
  type ProgressCopyId,
} from "@/components/banzai/progressPresentation";
import {
  progressLineFor,
  factsFromEvents,
  hasAnyFact,
...
function Chip({ label, value, locale }: { label: ProgressCopyId; value: string; locale: Locale }) {
  return (
    <span className="inline-flex items-baseline gap-[6px] rounded-[7px] border border-black/[0.07] bg-white/70 px-[9px] py-[4px]">
      <span className="font-mono text-[9.5px] uppercase tracking-[0.08em] text-ink-5">{progressCopy(label, locale)}</span>
...
function FactCards({ events, locale }: { events: ProgressEvent[]; locale: Locale }) {
  ...
  return (
    <div data-progress-facts="1" className="mt-[12px] flex flex-col gap-[10px]">
      <div className="flex items-center gap-[6px] font-mono text-[9.5px] tracking-[0.14em] text-ink-5">
        <Ico name="graph" size={12} className="text-bordo-soft" /> {progressCopy("block.factsHeading", locale)}
      </div>

      {f.entity && (
        <div data-fact="entity" className={`flex flex-wrap gap-[6px] p-[10px] ${CARD}`}>
          {f.entity.entity_id && <Chip locale={locale} label="chip.entity" value={f.entity.entity_id} />}
          {f.entity.entity_type && <Chip locale={locale} label="chip.type" value={f.entity.entity_type} />}
          {f.entity.artifact_type && <Chip locale={locale} label="chip.artifact" value={f.entity.artifact_type} />}
          {f.entity.protocol_scope && <Chip locale={locale} label="chip.scope" value={f.entity.protocol_scope} />}
          {f.entity.authority_requirement && <Chip locale={locale} label="chip.authority" value={f.entity.authority_requirement} />}
          {f.entity.requires_live_tool && <Chip locale={locale} label="chip.source" value={progressCopy("chip.live", locale)} />}
        </div>
      )}

      {f.package && (
        <div data-fact="package" className={`flex flex-wrap gap-[6px] p-[10px] ${CARD}`}>
          {f.package.source && <Chip locale={locale} label="chip.package" value={f.package.source} />}
          {f.package.primary_intent && <Chip locale={locale} label="chip.intent" value={f.package.primary_intent} />}
          {f.package.question_family && <Chip locale={locale} label="chip.family" value={f.package.question_family} />}
          {f.package.reason_code && <Chip locale={locale} label="chip.reasonCode" value={f.package.reason_code} />}
          {f.package.facts_count != null && <Chip locale={locale} label="chip.facts" value={String(f.package.facts_count)} />}
          {f.package.sample_size != null && <Chip locale={locale} label="chip.sample" value={String(f.package.sample_size)} />}
          {f.package.aggregation_method && <Chip locale={locale} label="chip.aggregation" value={f.package.aggregation_method} />}
          {f.package.depth && <Chip locale={locale} label="chip.depth" value={f.package.depth} />}
          {f.package.documentary_sources.map((s) => (
            <Chip key={`ps-${s}`} locale={locale} label="chip.source" value={s} />
          ))}
          {f.package.package_checksum && <Chip locale={locale} label="chip.checksum" value={f.package.package_checksum} />}
        </div>
      )}
        ...
        <div data-fact="sources" className={`flex flex-wrap gap-[6px] p-[10px] ${CARD}`}>
          {f.sources.map((s, i) => (
            <span key={`src-${i}`} className="inline-flex flex-wrap items-baseline gap-[6px]">
              {s.source_kind && <Chip locale={locale} label="chip.source" value={s.source_kind} />}
              {s.document_id && <Chip locale={locale} label="chip.doc" value={s.document_id} />}
              {s.document_status && <Chip locale={locale} label="chip.state" value={s.document_status} />}
              {s.seeded_entity && <Chip locale={locale} label="chip.entity" value={s.seeded_entity} />}
              {s.implementation_id && <Chip locale={locale} label="chip.implementation" value={s.implementation_id} />}
              {s.canonical_origin && <Chip locale={locale} label="chip.origin" value={s.canonical_origin} />}
              {s.artifact_version && <Chip locale={locale} label="chip.version" value={s.artifact_version} />}
              {s.artifact_sha256 && <Chip locale={locale} label="chip.sha256" value={s.artifact_sha256} />}
            </span>
          ))}
        </div>
      )}
        ...
        <div data-fact="tools" className={`flex flex-wrap gap-[6px] p-[10px] ${CARD}`}>
          {f.tools.map((t, i) => (
            <span key={`tool-${i}`} className="inline-flex flex-wrap items-baseline gap-[6px]">
              {t.tool_kind && <Chip locale={locale} label="chip.tool" value={t.tool_kind} />}
              {t.outcome && <Chip locale={locale} label="chip.outcome" value={t.outcome} />}
              {t.error_code && <Chip locale={locale} label="chip.code" value={t.error_code} />}
              {t.comparable_n != null && <Chip locale={locale} label="chip.n" value={String(t.comparable_n)} />}
              {t.operational_metric && <Chip locale={locale} label="chip.metric" value={t.operational_metric} />}
              {t.question_family && <Chip locale={locale} label="chip.family" value={t.question_family} />}
            </span>
          ))}
        </div>
      )}
    ...
    <div data-banzai-progress="1" className="flex items-start gap-[14px]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div data-progress-line="1" role="status" aria-live="polite" className="flex items-center gap-[10px] text-ink-4">
          <span className="sr-only">{progressCopy("block.srPrefix", locale)}</span>
          <span className="font-mono text-[12px] text-ink-3">{realizeProgressLine(line, locale)}</span>
        ...
        <FactCards events={events} locale={locale} />
        ...
        {synth && (
          <p data-progress-synthesis="1" className="m-0 mt-[12px] font-mono text-[11.5px] leading-[1.5] text-ink-4">
            {progressCopy("block.synthesis", locale)}
          </p>
        )}
        ...
        {onCancel && (
          <div className="mt-[12px]">
            ...
            >
              {progressCopy("block.cancel", locale)}
            </button>
          </div>
        )}
      </div>
...
export function BanzaiProgressMetrics({ metrics }: { metrics: ProgressMetrics | null | undefined }) {
  ...
  const rows: [ProgressCopyId, number | null][] = [
    ["metrics.firstProgress", m.timeToFirstProgressMs],
    ["metrics.firstVerifiedFact", m.timeToFirstVerifiedFactMs],
    ["metrics.validatedAnswer", m.timeToFinalValidatedAnswerMs],
    ["metrics.ttfb", m.ttfbMs],
  ];
  return (
    <section data-progress-metrics="1">
      <div className="mb-[10px] flex items-center gap-2 font-mono text-[10px] tracking-[0.14em] text-ink-5">
        <Ico name="graph" size={13} className="text-ink-5" /> {progressCopy("metrics.heading", locale)}
      </div>
      <dl className={`m-0 grid grid-cols-2 gap-x-[14px] gap-y-[7px] p-[14px] ${CARD}`}>
        {rows.map(([label, v]) => (
          <div key={label} className="flex flex-col">
            <dt className="font-mono text-[9.5px] tracking-[0.1em] text-ink-5">{progressCopy(label, locale).toUpperCase()}</dt>
            <dd className="m-0 font-mono text-[12.5px] text-ink-2">{formatProgressMs(v, locale)}</dd>
          </div>
        ))}
      </dl>
    </section>
