// BanzAI public adapter (ADR-037, ADR-044/045/046/047). RUST_WRAPPER_ONLY.
//
...
// "duration" (ADR-078) = an operational duration/metric answer (meta.terminal_kind ===
...
// ADR-078 — the operational duration/metric payload (meta.duration), snake_case→camelCase. The UI
...
export type KbAnswer = {
  ...
  degraded?: boolean;
  // Per-answer execution-path label (ADR-044 transparency): proves whether THIS answer was
  ...
  // ADR-078 — operational duration/metric. `answerType`/`terminalKind` echo the backend contract
...
// M2.19G.5C (ADR-073) — the mandatory post-synthesis authority validator blocked the model answer; the
...
const POST_VALIDATION_LIMIT =
  "Resposta em modo degradado: a resposta do modelo não passou a validação de autoridade (ADR-073); resposta determinística a partir das fontes do protocolo.";

// ADR-078 — an operational duration question with NO comparable data (or telemetry disabled). This is a
...
export function mapAskResponse(d: unknown): KbAnswer {
  ...
  // ADR-078 — operational duration/metric answer. `terminal_kind`/`answer_type` classify it; a SUCCESS
  ...
  if (docNotFound) {
    ...
    // ADR-078 — honest, distinct from "Evidência insuficiente": there simply is no comparable
  ...
  return {
    ...
    // ADR-078 — operational duration/metric echoes (present only for an operational answer).
...
export type AskJourney = {
  ...
  // M2.10A: the canonical document the operator is asking about ("ADR-002"), set when the question
