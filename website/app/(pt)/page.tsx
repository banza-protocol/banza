...
// phrase + public status · "Quem faz parte do protocolo" (Technical Registry) · "Três camadas. Uma
// interface." · Footer (from layout). The obsolete manifest-testing form, the extra value-proposition
// copy and the validation-journey section were removed; the single hero CTA opens the BanzAI validation
// mode. The Home carries only the SHORT protocol definition — the full introductory definition lives at
// /referencia/o-que-e. Fonts map to the site's next/font CSS variables; keyframes live in globals.css.
...
const HERO_INDICATORS: { label: string; glyph: React.ReactNode }[] = [
  {
    label: "Endpoints públicos",
    glyph: (
  ...
  {
    label: "Motores Rust determinísticos",
    glyph: (
  ...
  {
    label: "Resultados rastreáveis por evidência",
    glyph: (
...
export default function HomePage() {
  return (
    <div style={{ fontFamily: F_SANS, background: "#F1EBE0", color: "#1A1512", overflowX: "hidden" }}>
        ...
        <div data-hero-grid style={{ position: "relative", zIndex: 3, width: "100%", maxWidth: 1240, margin: "0 auto", padding: "0 clamp(18px,4vw,52px)", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(340px,0.72fr)", gap: "clamp(28px,4vw,64px)", alignItems: "center", minHeight: "clamp(300px,44vh,460px)" }}>
          ...
          <div data-hero-copy style={{ minWidth: 0, maxWidth: 620, position: "relative" }}>
            <div data-rise="" style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: "clamp(12px,1.5vw,18px)", animationDelay: ".05s" }}>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "#FBF0EE", border: "1px solid rgba(142,19,38,0.18)", padding: "8px 16px", borderRadius: 999, maxWidth: "100%" }}>
                ...
                <span style={{ fontFamily: F_MONO, fontSize: "clamp(10px,2.6vw,11.5px)", fontWeight: 600, letterSpacing: "0.12em", color: "#8B1428", whiteSpace: "nowrap" }}>PROTOCOLO FINANCEIRO ABERTO · v1.0</span>
              </div>
            </div>
            <h1 id="hero-title" style={{ fontFamily: F_DISPLAY, fontWeight: 600, fontSize: "clamp(32px,3.6vw,54px)", lineHeight: 1.06, letterSpacing: "-0.025em", color: "#1A1512", margin: "0 0 clamp(14px,1.4vw,20px)" }}>
              <span style={{ display: "block", overflow: "hidden", paddingBottom: ".04em" }}><span data-line="" style={{ animationDelay: ".14s" }}>Protocolo aberto e</span></span>
              <span style={{ display: "block", overflow: "hidden", paddingBottom: ".04em" }}><span data-line="" style={{ animationDelay: ".24s" }}>verificável de</span></span>
              <span style={{ display: "block", overflow: "hidden", paddingBottom: ".04em" }}><span data-line="" style={{ color: "#8B1428", fontStyle: "italic", animationDelay: ".34s" }}>interoperabilidade financeira.</span></span>
            </h1>
            ...
            <p data-rise="" style={{ fontSize: "clamp(15px,1.1vw,16.5px)", lineHeight: 1.6, color: "#5D5348", maxWidth: "54ch", margin: "0 0 clamp(18px,2vw,26px)", animationDelay: ".5s" }}>O BANZA cria uma linguagem comum para que operadores financeiros independentes interoperem através de regras públicas, conformidade demonstrável e evidência verificável — uma camada aberta que se acrescenta à interoperabilidade operacional já existente, tornando as regras, os testes e a evidência públicos e reproduzíveis por terceiros.</p>

            <ul data-rise="" aria-label="O que o protocolo oferece" style={{ listStyle: "none", display: "flex", flexWrap: "wrap", alignItems: "stretch", gap: "clamp(10px,1.4vw,22px)", margin: "0 0 clamp(20px,2.2vw,28px)", padding: 0, animationDelay: ".58s" }}>
              {HERO_INDICATORS.map((ind, i) => (
                <li key={ind.label} style={{ display: "flex", alignItems: "center", gap: "clamp(8px,0.8vw,11px)", minWidth: 0, flex: "1 1 180px" }}>
                  ...
                  <span style={{ display: "block", fontSize: "clamp(12px,1vw,13.5px)", lineHeight: 1.35, color: "#514639", fontWeight: 500 }}>{ind.label}</span>
            ...
            <div data-rise="" style={{ animationDelay: ".64s", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 14 }}>
              <Link href="/banzai?mode=validation" style={{ display: "inline-flex", alignItems: "center", gap: 12, background: "linear-gradient(180deg,#9C1B2F 0%,#7A1023 100%)", color: "#fff", textDecoration: "none", fontFamily: F_SANS, fontSize: 15.5, fontWeight: 600, letterSpacing: "0.01em", padding: "15px 28px", borderRadius: 13, boxShadow: "0 1px 0 rgba(255,255,255,0.18) inset,0 14px 28px -12px rgba(142,19,38,0.55)" }}>
                <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#fff" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l7 2.6v5.2c0 4.2-3 7.2-7 8.9-4-1.7-7-4.7-7-8.9V5.6z" /><path d="M9.5 12l1.8 1.8 3.4-3.6" /></svg>Validar operador no BanzAI<span aria-hidden="true" style={{ fontFamily: F_MONO }}>→</span>
              </Link>
              {/* WP1 — additive secondary CTA (outlined/neutral); primary above is unchanged and stays primary. */}
              <Link href="/whitepaper" style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "transparent", color: "#8B1428", textDecoration: "none", fontFamily: F_SANS, fontSize: 15.5, fontWeight: 600, letterSpacing: "0.01em", padding: "14px 26px", borderRadius: 13, border: "1px solid rgba(142,19,38,0.45)" }}>
                <svg aria-hidden="true" viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="#8B1428" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M6 3h9l4 4v14a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1z" /><path d="M14 3v5h5M8.5 13h7M8.5 16.5h7" /></svg>Ler o Whitepaper
              </Link>
            </div>
          </div>
          ...
          <div data-rise="" style={{ animationDelay: ".62s", minWidth: 0, background: "#FFFCF7", border: "1px solid rgba(218,204,180,0.9)", borderRadius: 24, boxShadow: "0 1px 2px rgba(26,21,18,0.03),0 24px 48px -34px rgba(26,21,18,0.16)", padding: "clamp(18px,1.6vw,26px)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 6 }}>
              <span style={{ fontFamily: F_MONO, fontSize: 10.5, letterSpacing: "0.16em", color: "#8A7F70" }}>INTEROPERABILIDADE</span>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontFamily: F_MONO, fontSize: 10.5, letterSpacing: "0.06em", color: "#8A7F70" }}><span aria-hidden="true" style={{ width: 6, height: 6, borderRadius: "50%", background: "#B8985F", flex: "none" }} />ILUSTRATIVO</span>
            </div>
            ...
            <div style={{ position: "relative", width: "100%", aspectRatio: "1/0.94" }}>
              ...
              <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", textAlign: "center", color: "rgba(184,152,96,0.95)", whiteSpace: "nowrap" }}><div style={{ fontFamily: F_DISPLAY, fontSize: "clamp(12px,1.15vw,15px)", letterSpacing: "0.03em", lineHeight: 1.2, fontWeight: 600 }}>Perfil BANZA v1.0</div><div style={{ fontFamily: F_MONO, fontSize: "clamp(8px,0.75vw,9.5px)", letterSpacing: "0.06em", lineHeight: 1.5, marginTop: 6, color: "rgba(184,152,96,0.82)" }}>Regras públicas<br />e versionadas</div></div>
              ...
              <div style={{ position: "absolute", top: 0, right: 0, width: "31%", background: "#FFFCF7", border: "1px solid rgba(218,204,180,0.9)", borderRadius: 14, boxShadow: "0 1px 2px rgba(26,21,18,0.03),0 16px 30px -26px rgba(26,21,18,0.22)", padding: "12px 8px", textAlign: "center" }}>
                <OperatorCard label="Operador B" sub="impl. B1"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#8B1428" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" style={{ display: "block", margin: "0 auto 7px" }}><path d="M3.5 9.5L12 5l8.5 4.5" /><path d="M5.5 10.5v7M9.5 10.5v7M14.5 10.5v7M18.5 10.5v7" /><path d="M3.5 19h17" /></svg></OperatorCard>
              </div>
              <div style={{ position: "absolute", bottom: 0, left: 0, width: "31%", background: "#FFFCF7", border: "1px solid rgba(218,204,180,0.9)", borderRadius: 14, boxShadow: "0 1px 2px rgba(26,21,18,0.03),0 16px 30px -26px rgba(26,21,18,0.22)", padding: "12px 8px", textAlign: "center" }}>
                <OperatorCard label="Operador C" sub="impl. C1"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#8B1428" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" style={{ display: "block", margin: "0 auto 7px" }}><rect x="7" y="2.6" width="10" height="18.8" rx="2.4" /><path d="M11 18.6h2" /></svg></OperatorCard>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, paddingTop: 14, borderTop: "1px solid #EAE1D1", fontSize: 12, color: "#8A7F70" }}>
              <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#8B1428" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ flex: "none" }}><path d="M12 3l7 2.6v5.2c0 4.2-3 7.2-7 8.9-4-1.7-7-4.7-7-8.9V5.6z" /><path d="M9.5 12l1.8 1.8 3.4-3.6" /></svg>Diagrama ilustrativo: operadores interoperam por regras e perfis comuns.
            </div>
          </div>
        </div>
      </section>
      ...
      <section aria-label="Estado público" style={{ position: "relative", background: "#F3EDE2", borderTop: "1px solid rgba(184,152,96,0.28)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "clamp(20px,2.4vw,32px) clamp(18px,4vw,52px)" }}>
          <div data-reveal style={{ display: "inline-flex", alignItems: "center", gap: 12, fontFamily: F_DISPLAY, fontSize: "clamp(18px,1.8vw,24px)", fontWeight: 600, letterSpacing: "-0.01em", color: "#1A1512" }}>
            ...
            Aberto. Auditável. Verificável.
          </div>
          <HeroStatusBar lastVerifiedAt={LAST_VERIFIED_AT} />
        </div>
      </section>

      {/* ============ QUEM FAZ PARTE DO PROTOCOLO (TECHNICAL REGISTRY) ============ */}
      <section aria-label="Registo técnico" style={{ position: "relative", background: "linear-gradient(180deg,#F3EDE2 0%,#F5F0E6 100%)", borderTop: "1px solid rgba(184,152,96,0.28)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "clamp(44px,5vw,80px) clamp(18px,4vw,52px) clamp(48px,5vw,72px)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 14, marginTop: "clamp(26px,3vw,40px)" }}>
            <Link href="/registo-tecnico" style={{ display: "inline-flex", alignItems: "center", gap: 12, background: "linear-gradient(180deg,#9C1B2F 0%,#7A1023 100%)", color: "#fff", textDecoration: "none", fontFamily: F_SANS, fontSize: 15.5, fontWeight: 600, letterSpacing: "0.01em", padding: "15px 26px", borderRadius: 13, boxShadow: "0 1px 0 rgba(255,255,255,0.18) inset,0 14px 28px -12px rgba(142,19,38,0.55)" }}>
              <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#fff" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6h16M4 12h16M4 18h10" /></svg>Consultar o Registo Técnico<span aria-hidden="true" style={{ fontFamily: F_MONO }}>→</span>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section aria-labelledby="layers-title" style={{ position: "relative", background: "linear-gradient(180deg,#F3EDE2 0%,#F1EBE0 100%)", borderTop: "1px solid rgba(184,152,96,0.28)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "clamp(44px,5vw,80px) clamp(18px,4vw,52px)" }}>
          <div data-reveal style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <span style={{ fontFamily: F_MONO, fontSize: 11, letterSpacing: "0.18em", color: "#7A7263" }}>ARQUITECTURA · TRÊS CAMADAS</span>
          </div>
          <h2 id="layers-title" data-reveal style={{ fontFamily: F_DISPLAY, fontWeight: 600, fontSize: "clamp(26px,3vw,42px)", lineHeight: 1.06, letterSpacing: "-0.02em", color: "#1A1512", margin: "0 0 clamp(8px,1vw,12px)" }}>Três camadas. <span style={{ color: "#8B1428", fontStyle: "italic" }}>Uma interface.</span></h2>
          <p data-reveal style={{ fontSize: "clamp(14px,1vw,15.5px)", lineHeight: 1.6, color: "#5D5348", maxWidth: "72ch", margin: "0 0 clamp(24px,3vw,36px)" }}>O BANZA separa as regras do protocolo, a certificação técnica e a operação de um scheme. O BanzAI é a interface transversal de consulta e validação; não constitui uma quarta camada.</p>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,280px),1fr))", gap: "clamp(14px,1.6vw,20px)" }}>
            {[
              { tag: "Camada 1", name: "BANZA · Protocolo", body: "Regras públicas e versionadas para perfis, contratos, schemas, identidade, discovery, metadata assinada, trust, revogação e federação. O BANZA não é banco, PSP, carteira ou operador e não movimenta fundos." },
              { tag: "Camada 2", name: "Certificação de Conformidade e Interoperabilidade", body: "Avalia implementações específicas contra profiles e versões públicas através de conformidade, interoperabilidade, trust e evidência verificável. Os motores Rust determinam os resultados, e os estados técnicos podem ser publicados no Registo Técnico. Não constitui licença, admissão num esquema ou autorização regulatória." },
              { tag: "Camada 3", name: "Esquemas operacionais independentes", body: "Esquemas construídos sobre o protocolo segundo as suas próprias regras e autorizações. O primeiro é o Esquema Operacional Banzami, com a Banzami como operadora designada do esquema — preparação regulatória em curso e pagamentos reais desactivados." },
            ].map((c) => (
              <div key={c.tag} data-reveal style={{ background: "#FFFCF7", border: "1px solid rgba(218,204,180,0.9)", borderRadius: 18, padding: "clamp(18px,1.8vw,26px)", boxShadow: "0 1px 2px rgba(26,21,18,0.03),0 18px 36px -32px rgba(26,21,18,0.16)" }}>
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 34, height: 26, padding: "0 8px", borderRadius: 8, background: "#FBF0EE", border: "1px solid rgba(142,19,38,0.18)", fontFamily: F_MONO, fontSize: 12, fontWeight: 600, letterSpacing: "0.04em", color: "#8B1428", marginBottom: 14 }}>{c.tag}</span>
                <div style={{ fontFamily: F_SERIF, fontSize: "clamp(15px,1.1vw,17px)", fontWeight: 600, lineHeight: 1.28, color: "#1A1512", marginBottom: 8 }}>{c.name}</div>
                <p style={{ fontSize: 13.5, lineHeight: 1.6, color: "#5D5348", margin: 0 }}>{c.body}</p>
              </div>
            ))}
          </div>
          ...
          <div data-reveal style={{ display: "flex", alignItems: "flex-start", gap: 12, marginTop: "clamp(16px,1.8vw,22px)", padding: "clamp(14px,1.4vw,18px) clamp(16px,1.6vw,22px)", background: "rgba(255,252,247,0.7)", border: "1px dashed rgba(184,152,96,0.5)", borderRadius: 14 }}>
            ...
            <p style={{ fontSize: 13.5, lineHeight: 1.6, color: "#453C33", margin: 0 }}><strong style={{ color: "#1A1512" }}>BanzAI — interface transversal.</strong> Permite consultar o protocolo, iniciar a validação técnica das implementações publicadas por operadores e interpretar os resultados. Os motores Rust executam e determinam os veredictos; o Qwen apenas explica. O BanzAI não é uma camada nem uma autoridade.</p>
          </div>
        </div>
      </section>
    </div>
