...
// (Layer 2, ADR-032/065/066). It replaces the former redirect into the reference chapter. It presents the
...
// The three objects of the L2 model (ADR-032): the yardstick, the subject, the verdict.
const CHAIN = [
  {
    k: "PERFIL",
...
// Exactly what a Certification Record binds to (ADR-032).
const BINDING = [
...
// The closed, Rust-decided certification-state machine (ADR-032). Only CERTIFIED reads as valid.
const STATES = [
...
// A closed enum of machine-readable reason codes decided only by Rust (ADR-032/066). Illustrative subset.
const REASON_CODES = [
...
// The three distinct determinations that never imply one another (ADR-005 non-propagation).
      ...
      <Section tone="paper2">
        <Container width="site" data-reveal>
          ...
          <p className="mb-6 max-w-[80ch] text-[15px] leading-[1.7] text-ink-4">
            ...
            fetch em Rust — nunca pelo navegador (ADR-034). O resultado é específico da implementação, do
