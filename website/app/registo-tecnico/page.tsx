...
// The canonical certification states published by the registry (ADR-066). Only CERTIFIED reads as valid.
const STATES = [
...
export default async function RegistoTecnicoPage() {
  ...
  // Não existe ainda um store nem rota máquina de registos de certificação (ADR-066); o valor mantém-se 0
  ...
  // The closed set the search box filters over (client-side only; never fetches a target — ADR-068).
      ...
      <Section tone="paper2">
        <Container width="site" data-reveal>
          ...
          <p className="mt-4 text-[13px] leading-[1.6] text-ink-5">
            ...
            Estados canónicos de certificação (ADR-066):{" "}
            {STATES.map((st, i) => (
              <span key={st.s}>
