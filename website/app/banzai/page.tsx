...
// Canonical, single public route for BanzAI — the native protocol agent (ADR-042). ONE app, three modes
...
// (ADR-040). Official validation is ENDPOINT-ORIGINATED (ADR-038): the target is resolved from the closed
...
//
// M2.19G.4 (ADR-042) — the operator and implementation CONTEXTS are addressable route segments under
...
export const metadata: Metadata = {
  ...
  description:
    "BanzAI é a interface humana primária e transversal entre humanos/operadores e o protocolo BANZA (ADR-042): interpreta pedidos, consulta a referência, orienta a implementação, encaminha para os motores verificáveis e explica os resultados. Tem três modos no mesmo espaço — perguntar ao BanzAI, validar um operador avaliando uma das suas implementações publicadas (jornada determinística de 9 etapas) e o onboarding de operadores (ADR-040). A validação oficial utiliza exclusivamente artefactos obtidos dos endpoints públicos da implementação seleccionada, resolvida no Registo Técnico e obtida por uma camada segura de fetch em Rust — nunca pelo navegador (ADR-038). Responde com base nas fontes locais do protocolo através de inferência local (Qwen, on-host), sem chamadas a modelos externos; invoca motores Rust e ajuda a preparar evidência verificável. Não é fonte normativa: não certifica, não aprova operadores, não emite licenças, não publica operadores, não movimenta fundos e não substitui os motores nem a governação do protocolo.",
