 ...
 *  `NOT_APPLICABLE` is a DISPLAY status derived from a step's `applicability` (ADR-077) — a step out of
...
/** ADR-077 profile applicability of a step, orthogonal to its result.status. */
export type Applicability = "REQUIRED" | "OPTIONAL" | "NOT_APPLICABLE";

// ─────────────────────────────────────────────────────────────────────────────
// M2.19G.1 (ADR-068) — server-issued receipts. The endpoint-originated official journey no longer
...
// by construction and protocol fetches are counted as `protocol_fetch_count` (ADR-068 §4.8).
// ─────────────────────────────────────────────────────────────────────────────

/** A single endpoint-originated step receipt (ADR-068 §30) — verdict bound to its exact public origin. */
export interface ServerOperationReceipt {
  ...
  workflow: string;
  step: string;
  /** ADR-077: whether this step is in scope for the declared profile. NOT_APPLICABLE steps are sealed
...
/** The aggregate endpoint-originated journey receipt (ADR-068 §31). Certification Readiness (READY |
...
export interface ServerJourneyReceipt {
  ...
  certified: false;
  /** ADR-077: Rust-decided step→applicability map + which steps are out of scope for this profile. */
 ...
 *  ADR-077: NOT_APPLICABLE steps are out of scope and excluded — they never contribute a verdict and
