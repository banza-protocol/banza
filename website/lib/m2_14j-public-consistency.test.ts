...
// M2.14J — public-surface final consistency (ADR-054). Source-level contract over the website
// surfaces reframed in this phase: ADR-052/053/054 are publishable on /decisoes; the primary
// human-operator interface is the leading definition (agente stays secondary); the 4-clause phrase is
// complete; the operador-zero copy matches the live (active) subdomain; and no public copy makes a
// POSITIVE forbidden claim (legitimate negations are fine).

const root = join(__dirname, "..");
const read = (p: string) => readFileSync(join(root, p), "utf8");

describe("M2.14J — ADR-052/053/054 are published on /decisoes", () => {
  for (const [id, slug] of [
    ["ADR-052", "adr-052"],
    ["ADR-053", "adr-053"],
    ["ADR-054", "adr-054"],
  ] as const) {
    it(`${id} is registered and resolvable`, () => {
      const d = getDecision(slug);
      expect(d, `${id} must be registered`).toBeTruthy();
      expect(d!.id).toBe(id);
      expect(d!.state).toBe("activo");
      // the mirrored body exists in the Docker build context.
...
describe("M2.14J — primary human-operator interface is the leading definition", () => {
  it("the /banzai page metadata leads with the primary interface", () => {
    ...
    // M2.19G.4 (ADR-070): the single sr-only H1 moved to the shared app/banzai/layout.tsx so it heads
...
describe("M2.19G — SVG-P-057 overview is the three-layer architecture with BanzAI transversal", () => {
