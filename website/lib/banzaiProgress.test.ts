...
describe("§9 SAFE facts-available projection", () => {
  it("extracts entity / sources / tools / package from the event payloads (safe ids/enums/counts/hashes)", () => {
    const events = [
      ...
      evt("FACTUAL_PACKAGE_READY", { source: "operational_telemetry", primary_intent: "operational_metric", facts_count: 4, documentary_sources: ["ADR-036"], tools_called: ["METRICS_QUERY"], sample_size: 3, aggregation_method: "median", package_checksum: "deadbeef" }),
    ];
    ...
    expect(f.package?.documentary_sources).toEqual(["ADR-036"]);
  });
