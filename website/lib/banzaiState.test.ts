...
describe("parseBanzaiState — validated URL-state for the single BanzAI app", () => {
  ...
  it("resolves mode ask | validation | onboarding (only exact literals opt in; M2.19G.3/ADR-037)", () => {
  ...
  it("seeds the Fase 0 ids from the (closed, shape-checked) target/implementation deep link (M2.19G.3B / ADR-036)", () => {
  ...
  // M2.19G.4 (ADR-036) — the navigable-context path seed. The route segments (operador/[operatorId]/
  ...
  it("ignores an off-shape / injected operator segment and falls back to the global context (no URL ever)", () => {
    for (const bad of ["../secret", "javascript:alert(1)", "https://attacker.example/x", "Operator Zero", "UPPER"]) {
      ...
      // (ADR-036 — the registry is the only selection source).
