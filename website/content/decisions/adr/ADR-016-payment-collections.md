# ADR-016 — Payment Collections (split / shared / group payments)
...
**Supersedes:** None  
**Extends:** ADR-014 (Payment Intent), ADR-017 (Wallet/account payments & refund source), ADR-012 (QR), ADR-013 (Payment Links)  
**See also:** ADR-005 (Protocol-first), [BANZA-PROTOCOL-VS-OPERATOR-POLICY](../../docs/governance/BANZA-PROTOCOL-VS-OPERATOR-POLICY.md), [spec/collections.md](../../spec/collections.md)
...
> protocol first (ADR-005); operators consume it downward (protocol → operator →
> SDK → apps).
...
is the inversion ADR-005 forbids: a structural concept (a group of related
...
Collection ──1:N──▶ CollectionShare ──▶ PaymentIntent ──▶ Transfer ──▶ Ledger
   (composite obligation)   (one payer)     (ADR-014)      (ADR-017)   (double-entry)
```
...
- **Forbidden:** `Collection → Ledger`. **Required path:** `Collection →
  ...
  Collections.
- Refunds follow ADR-017 **per settled share** (source-aware), never at the
  Collection level.
...
(`contracts/federation/*`, ADR-040 federation trust evaluation on the
certificate-free open trust model of ADR-038) **without** changing the Collection
...
- The reference operator's app-side split prototype (pre-protocol,
  disabled-by-default; the reference operator's ADR-003) is superseded by the
