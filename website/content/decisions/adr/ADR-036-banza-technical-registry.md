# ADR-036 — BANZA Technical Registry

- **Status:** Accepted
- **Date:** 2026-07
- **Related:** ADR-034 (Conformance & Interoperability Certification model), ADR-035 (closed
  certification-state machine), ADR-003 (three-layer architecture — L2), ADR-006 (Banzami Operational
  Scheme — the L3 participant directory), ADR-004 (certification ≠ admission ≠ authorisation), ADR-027
  (open trust model without a central CA), ADR-033 (operator self-publication)

---

## Context

ADR-034 makes an L2 certification a concrete, hash-bound `CertificationRecord`. Those records need a
...
with a fixed, verifiable contract and a hard boundary from the L3 scheme's participant directory (ADR-006),
...
| ID | Decision |
|----|----------|
| **D-065-01** | **What the registry holds.** The Technical Registry indexes exactly the L2 objects of ADR-034: published `InteroperabilityCertificationProfile`s (by `profile_id`+`profile_version`+`profile_hash`), `CertificationRecord`s (by `record_id`+`record_hash`), and the **revocation list** (signed, dated). It holds no funds, no accounts, no personal data and no scheme membership — only technical certification facts. |
| **D-065-02** | **Public verification, no account.** Every registry entry is verifiable by any third party with **no BANZA account, no scheme membership and no privileged endpoint**: a reader re-runs the profile's public vectors against the implementation's artifacts and reproduces the record's evidence hashes, and checks the record against **root-signed protocol metadata** (ADR-027). There is no CA and no certificate chain. |
| **D-065-03** | **Registry ≠ scheme directory.** The Technical Registry (L2) is **independent of** the Banzami (or any) scheme's **participant directory** (L3). Presence in the registry means "this implementation holds this certification record" and **never** "this entity is admitted to a scheme" or "authorised by a regulator" (ADR-004 D-061-04/05, ADR-006 D-060-06). The two are separate artifacts with separate owners; neither is derivable from the other. |
| **D-065-04** | **Fail-closed reads.** A registry entry that is missing, unreadable, expired, suspended or revoked is reported as **not a valid certification**, never as valid (ADR-034 D-064-06, ADR-027 INV-OTE-005/006). A verifier that cannot reproduce a record's hashes MUST treat it as invalid, not trusted. |
| **D-065-05** | **Rust-owned, append-mostly, immutable records.** The registry is written only by the Rust engine after it validates a record (ADR-034 D-064-04); no human and no model writes, edits or removes an entry, and no configuration turns an invalid entry valid. A record is **immutable** once published; a change of standing is expressed as a **new** record or a **revocation** entry, never by mutating history (ADR-035). |
| **D-065-06** | **Neutral + operator-agnostic.** The registry lists implementations from **any** legally-eligible party on identical terms; it encodes no operator brand as a privileged entry, no reduced-profile path and no reserved slot (ADR-001, ADR-007). Banzami's own certified implementation, if any, appears through exactly the same public path as any other (ADR-007 conflict-of-interest controls). |
...
- **L3 scheme participant directory** (ADR-006) — who a *scheme* admits, under the scheme's own rules; a
  separate artifact, never implied by registry presence.
- **Regulatory authorisation** (ADR-005) — the regulator's determination; BANZA is not a party and the
...
**Untouched.** No financial invariant; the open trust model (ADR-027/040), Rust-sole-authority (ADR-043) and
operator neutrality (ADR-001) all stand.
...
- **Governance note:** the repo-canonicalization framing (this repo's `services/banzai-api` as the
  canonical runtime) is governed separately by ADR-042; this amendment covers naming only.

## References

- ADR-034 (certification model), ADR-035 (state machine), ADR-006 (scheme directory), ADR-004 (separation)
