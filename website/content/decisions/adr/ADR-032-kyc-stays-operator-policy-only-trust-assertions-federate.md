# ADR-032 — KYC stays operator policy; only trust assertions federate
...
verifies identity.** Per ADR-001's litmus test, a concept belongs in the protocol
...
(ADR-031), not an identity-mechanics problem.
...
A future ADR (extending ADR-031) would define the wire contract if pursued.
