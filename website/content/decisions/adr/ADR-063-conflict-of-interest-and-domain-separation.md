# ADR-063 — Conflict of Interest, Infrastructure Separation and Key Separation (Creator == First Operator)

- **Status:** Accepted
- **Date:** 2026-07
- **Milestone:** M2.19C
- **Related:** ADR-059 (three-layer institutional architecture), ADR-060 (Banzami Operational Scheme —
  designated operator; BANZA ≠ Banzami), ADR-061 (certification ≠ admission ≠ authorisation), ADR-062
  (regulatory-state boundary + real-money activation gate), ADR-028 (private keys never on serving
  infrastructure), ADR-037 (Rust-first official engines), ADR-038 (open protocol trust model without CA),
  ADR-042 (PostgreSQL as protocol-state store)

---

## Context

ADR-043 names **Banzami — Tecnologia e Serviços, Lda.** as the creator and initial institutional
maintainer of the open protocol, and ADR-060 designates the same company as the operator of the first
...
ADR-060 already fixed the principle (D-060-05: no self-privilege) and ADR-059 already fixed that the three
...
**Untouched.** No financial invariant. The open trust model (ADR-038), keys-never-on-serving-infrastructure
(ADR-028), Rust-sole-authority and Qwen-never-decides (ADR-037/059), the regulatory-state boundary
(ADR-062) and BANZA's operator neutrality (ADR-001/003/059/060) all stand. This ADR decides how the
...
- ADR-059 (three-layer architecture), ADR-060 (Banzami Operational Scheme), ADR-061 (certification ≠
  admission ≠ authorisation), ADR-062 (regulatory-state + real-money gate), ADR-028 (keys never on serving
  infrastructure), ADR-037 (Rust-first engines), ADR-038 (open trust model without CA), ADR-042 (PostgreSQL
