...
- **Status:** Accepted
- **Date:** 2026-07
- **Companion:** ADR-027 — Open Protocol Trust and Participation Model (the trust evaluation itself)
...
- The financial invariants (`INV-LEDGER-*`, `INV-WALLET-*`, `INV-SETTLE-*`, `INV-IDEM-*`, `INV-RECON-*`,
  ...
  correctness means.
- Operator technology neutrality is untouched (ADR-001, ADR-043): an operator may implement the protocol
  in any technology.
...
- The evaluation performed over the artifacts this ADR governs — Public Registry metadata, signed
  ...
  verification, and revocation/fail-closed — is defined by the companion ADR-027, not here.
