...
**Supersedes:** None  
**Extends:** ADR-001 (Open protocol), ADR-003 (Protocol/operator separation), ADR-002 (Ecosystem naming)  
**See also:** [BANZA-PROTOCOL-VS-OPERATOR-POLICY](../../docs/governance/BANZA-PROTOCOL-VS-OPERATOR-POLICY.md), ADR-016 (Payment Collections)

---

## Context

ADR-001/ADR-003 established that BANZA is the open protocol and that operators
...
1. BANZA defines `Collection` (ADR-016). → The reference operator implements a
...
1. **Protocol:** ADR-016 defines `Collection`, `CollectionShare`, `CollectionRule`,
   their states, events, and relationship to `PaymentIntent` / `Transfer` /
   ledger. (Proposed.)
2. **Operator:** once ADR-016 is Accepted, the reference operator implements
...
disabled-by-default flag and documented as a prototype that anticipates ADR-016,
not as an implementation of it. (Operator-side classification: the reference operator's ADR-003.)
