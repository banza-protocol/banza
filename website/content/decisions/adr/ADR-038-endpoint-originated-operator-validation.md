# ADR-038 — Endpoint-originated operator validation

- **Status:** Accepted
- **Date:** 2026-07
- **Related:** ADR-042 (BanzAI as the single human-operator interface), ADR-003 (three-layer
  institutional architecture), ADR-006 (Banzami Operational Scheme — designated operator),
  ADR-004 (technical certification ≠ scheme admission ≠ regulatory authorisation),
  ADR-034/065/066 (L2 conformance & interoperability certification, technical registry, closed
  certification-state machine), ADR-041 (Operador Zero read-only reference implementation),
  ADR-043 (Rust-first engines), ADR-027 (open trust model, no central CA)

---

## Context

BanzAI's validation mode (ADR-041) lets a human validate an implementation of the BANZA protocol
...
The **BANZA Technical Registry** (ADR-036), or a closed technical target registry, provides the eligible
...
Operador Zero (ADR-041) is the initial canonical example, but it receives **no shortcut, official
...
certification; technical certification is not scheme admission nor regulatory authorisation (ADR-004).
