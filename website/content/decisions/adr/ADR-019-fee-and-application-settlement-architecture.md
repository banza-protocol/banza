# ADR-019 — Generic Fee & Application-Settlement Architecture
...
**Supersedes:** None
**Extends:** ADR-014 (Payment Intent), ADR-016 (Payment Collections), ADR-006 (Double-entry ledger), ADR-017 (Wallet/account payments)
**See also:** ADR-005 (Protocol-first), ADR-052 (Reference operator simulator), [BANZA-PROTOCOL-VS-OPERATOR-POLICY](../../docs/governance/BANZA-PROTOCOL-VS-OPERATOR-POLICY.md)
...
later settlement to a beneficiary". Per ADR-005 that is the inversion to avoid: a
...
- Realized as **one additional ledger leg** in the same double-entry posting the
  fulfilment already produces (ADR-006): the payee is credited the **net**, an
...
- **Crowdfunding** — *Keep-What-You-Raise* → settle immediately; *All-or-Nothing*
  → settle only when the campaign closes successfully (else refund via ADR-017).
...
## Ledger (ADR-006 — unchanged invariants)
...
## Implementation sequencing (ADR-005, protocol → operator → SDK → apps)
