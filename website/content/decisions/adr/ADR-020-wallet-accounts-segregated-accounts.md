# ADR-020 — Wallet Accounts (Segregated Accounts within a Wallet)
...
**Supersedes:** None
**Extends:** ADR-019 (Fee & Application-Settlement), ADR-017 (Wallet/account payments), ADR-006 (Double-entry ledger), ADR-014 (Payment Intent)
**See also:** ADR-005 (Protocol-first), ADR-052 (Operador Zero simulator), ADR-053 (Operator Zero Only demo policy), [BANZA-PROTOCOL-VS-OPERATOR-POLICY](../../docs/governance/BANZA-PROTOCOL-VS-OPERATOR-POLICY.md)

---

## Context

ADR-019 defines an **Application Settlement** that pays a beneficiary from *net
...
to move. That is precisely the inversion ADR-005 forbids: a parallel,
unreconcilable, uncertifiable ledger living inside an application, and an
application *computing money*.

Per ADR-005 this structural concept — *how value is segregated within an owner's
...
4. **Ledger postings always reference an `account_id`** (ADR-006 already posts to
...
- **Settlement (ADR-019):** the settlement `source` becomes an `account_id`,
...
- **Multiple wallets per owner/currency** — rejected: a wallet is, by ADR-017, the
...
- **Application-side sub-balances** — rejected by ADR-005 (a parallel ledger).
