...
```
          ...
          Payment Link  Dynamic QR  Static QR   Deep Link
                 └────────────┴────┬────┴─────────────┘
                                  ▼
                         one destination
              (payee wallet, MAY be a Wallet Account — ADR-019)
                                  ▼
...
|---|---|
| `PAYMENT_LINK` | A URL (ADR-017) resolving to the session. |
| `DYNAMIC_QR` | A QR (ADR-016) bound to one session; MAY expire; MAY be fixed-amount; single- or multi-use per policy. |
| `STATIC_QR` | A QR (ADR-016) bound to a permanent session or a session template (campaign, shop, event, @handle); MAY allow an open amount; **the destination is still resolved by the operator** — a static QR never lets an app invent a destination. |
...
- The session **adds** an optional `destination_account_ref` — the segregated
  **Wallet Account** (ADR-019) to credit (e.g. a campaign account). Absent ⇒ the
...
8. The destination MAY be a Wallet Account (ADR-019); the operator resolves it —
...
Payment Session
  ...
  destination    = the campaign Wallet Account (ADR-019)
...
(its operator implementation is the reference operator's ADR-020, reframed as the implementation of
...
- Existing Payment Links (ADR-017) and QRs (ADR-016) remain valid. A legacy
...
2. **Operator (reference-operator ADR-020):** `payment_sessions`; bind link + dynamic/static
...
- [`contracts/payment-sessions/payment-session-interface.schema.json`](../../contracts/payment-sessions/payment-session-interface.schema.json)
- [`contracts/payment-sessions/payment-session.schema.json`](../../contracts/payment-sessions/payment-session.schema.json)
