...
## m2-19g-public-surface-canonical-check: Capstone aggregate sweep over the rendered public surface — zero retired framings as positive claims (negation-aware), three-layer vocabulary present (Camada 1/2/3 + Esquema Operacional + BanzAI transversal), qualified hero with no absolute "sem acordos bilaterais" (M2.19G)
m2-19g-public-surface-canonical-check:
	@bash tools/check-m2-19g-public-surface-canonical.sh
...
m2-19g2-home-canonical-check:
	@bash tools/check-m2-19g2-home-canonical.sh

## m2-19g2-o-que-e-removal-check: Verify the /o-que-e route removal — no route dir, no redirect/rewrite/alias source, no sitemap/SW/internal-link, every "Ler a referência" → /referencia, /referencia/o-que-e the single canonical definition, BanzAI grounding base carries no /o-que-e source/href (M2.19G.2 §27-28)
m2-19g2-o-que-e-removal-check:
	@bash tools/check-m2-19g2-o-que-e-removal.sh

## m2-19g2-readiness-check: M2.19G.2 capstone — runs the two G2 guards and asserts the §42 static metric block (home_primary_ctas=1, all legacy_o_que_e_*=0, canonical_o_que_e_sources=1, …); runtime metrics delegated to public-edge QA (M2.19G.2 §42)
...
## banzai-m2-19g-semantic-regression-check: M2.19G.1 — the M2.19G three-layer public surface stays intact on the touched pages; 0 reintroduced retired terms (m2_19g_semantic_regressions=0)
banzai-m2-19g-semantic-regression-check:
	@bash tools/check-banzai-m2-19g-semantic-regression-check.sh
