#!/usr/bin/env bash
#
# M2.19G.1 (ADR-068 §31) — JourneyReceipt origin-fields schema guard (§37, invariant 22).
#
...
echo "== banzai-journey-receipt-origin-check (M2.19G.1 / ADR-068 §31) =="
...
if [ -f "$VALIDATE" ]; then
  # ADR-076: the journey receipt is built as `const journeyReceipt = { … }` (so it can be persisted via
