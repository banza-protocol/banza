#!/usr/bin/env bash
#
# BanzAI mandatory post-synthesis authority validator guard (M2.19G.5C, ADR-073).
#
# ADR-073 makes the post-synthesis authority validator a MANDATORY publish-gate step: after the single
...
if grep -qE 'post_validation_' "$PIPE"; then
  ...
  grep -qE 'post_validation_\$\{verdict\.reason\}|post_validation_unsupported_claim|post_validation_contradicts_deterministic' "$PIPE" \
    && ok "the three ADR-073 gate reasons are present (authority/leak, unsupported_claim, contradicts_deterministic)" \
    || fl "expected the ADR-073 gate reasons (post_validation_<verdict>, _unsupported_claim, _contradicts_deterministic)"
else
...
if [ "$fail" -ne 0 ]; then
  echo
  echo "banzai-post-synthesis-validation: FAIL — the mandatory post-synthesis publish gate (ADR-073) drifted."
  exit 1
fi
echo
echo "banzai-post-synthesis-validation: ✓ postValidate gates the grounded publish path (after synthesis, before groundedAnswer/cache/return); three-state validation_status derived from the post_validation_ emitter (ADR-073)"
