...
#   * the ADR-073 post-synthesis validator + the Inc.4 claim/citation verification run BEFORE any
...
# ── static: ADR-073 post-validate + Inc.4 verification run BEFORE any FINAL_VALIDATED with prose. ──────
...
if [ -n "$claim_line" ] && [ -n "$postval_line" ] && [ -n "$grounded_line" ] && [ "$claim_line" -lt "$postval_line" ] && [ "$postval_line" -lt "$grounded_line" ]; then
  ok "ADR-073 postValidate + Inc.4 verification precede building the grounded (prose) answer (claim@$claim_line < postValidate@$postval_line < groundedAnswer@$grounded_line)"
else
