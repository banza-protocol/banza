...
if [ -f "$KB" ]; then
  ...
  grep -q 'Gerado por Qwen local' "$KB" && ok "chat adapter emits a per-answer 'Gerado por Qwen local' label" \
    || fail "banzaiKb.ts must emit a per-answer status label (Gerado por Qwen local)"
