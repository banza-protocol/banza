...
grep -qE 'Ver receipt'            "$MODE" && ok '"Ver receipt" action present'            || fl 'missing "Ver receipt" action'
grep -qE 'Explicar este resultado' "$MODE" && ok '"Explicar este resultado" action present' || fl 'missing "Explicar este resultado" action'
grep -qE 'Executar novamente'      "$MODE" && ok '"Executar novamente" action present'      || fl 'missing "Executar novamente" action'
...
if printf '%s\n' "$block" | grep -qE 'Ver receipt'; then
  # The Ver receipt button in that block must carry the primaryBtn class.
  if printf '%s\n' "$block" | grep -E 'Ver receipt' | grep -qE 'primaryBtn'; then
  ...
  if printf '%s\n' "$block" | grep -qE 'Executar esta etapa'; then
...
grep -qE 'Executar esta etapa' "$MODE" && ok '"Executar esta etapa" exists for the pending-step case' || fl 'missing "Executar esta etapa" primary for a pending step'
