#!/usr/bin/env bash
#
# Operador Zero full E2E protocol validation guard (ADR-035, M2.13A).
#
...
#   - the BanzAI official validation journey is endpoint-originated (M2.19G.1, ADR-034): it obtains every
...
# ── 6. The BanzAI official validation journey is endpoint-originated (ADR-034) ────────────────────
echo "e2e: BanzAI validation journey is endpoint-originated (ADR-034), not bundled/apex reads…"
...
# (b) M2.19G.1 (ADR-034 §4.4) — the 9-step official journey is ENDPOINT-ORIGINATED: it calls the Rust
...
# (c) ADR-034 §4.4/§4.5 — no bundled manifest, local fixture or readArtifact is a VERDICT SOURCE in the
...
if grep -qE 'readArtifact|loadManifestFixtures|readFixture' "$VJOURNEY"; then
  bad "the official journey must not read bundled/fixture artifacts as a verdict source (ADR-034 §4.4)"
else
