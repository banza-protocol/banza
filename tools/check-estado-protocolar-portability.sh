...
# (protocol state), defined by its semantics; the ADR-042 data boundary (protocol state, not financial
...
if check "$REF"; then
  ...
  echo "Result: ✗ §5 presents PostgreSQL as more than implementation (see docs/reference/BANZA_SVG_REGISTRY.md, ADR-042)"
  exit 1
fi
