#!/usr/bin/env bash
# operator-zero-only-architecture-check (ADR-035, M2.14B).
#
...
echo "== operator-zero-only-architecture-check (M2.14B / ADR-035) =="

# 1. The architectural decision exists.
ADR=$(ls decisions/adr/ADR-035-*.md 2>/dev/null | head -1)
[ -f "$ADR" ] && grep -q "sole canonical demo operator" "$ADR" && ok "ADR-035 present and states the policy" || bad "ADR-035 missing or does not state the policy"
