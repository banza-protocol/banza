#!/usr/bin/env bash
#
# M2.19G.1 (ADR-034 §4.7 / Consequences) — Rust fetch authority guard (§37, invariant 24).
#
...
echo "== banzai-rust-fetch-authority-check (M2.19G.1 / ADR-034 §4.7) =="
...
echo "banzai-rust-fetch-authority-check: ✓ official fetch is Rust (banza-fetcher); browser only POSTs closed ids (ADR-034 §4.7)"
