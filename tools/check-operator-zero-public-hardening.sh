#!/usr/bin/env bash
#
# Operador Zero public-surface hardening guard (ADR-035, M2.12C).
#
...
# M2.19G (ADR-035): ch.9 was rewritten from the simulator model to the READ-ONLY reference model.
...
# is allowed ONLY in ADR-035, per the surgical identity-check allowlist). The brand is assembled below
