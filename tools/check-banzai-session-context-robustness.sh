#!/usr/bin/env bash
#
# BanzAI Session Context Robustness Guard (M2.11A; reframed by ADR-076 §D-076-01/02).
#
# The first journey model treated navigation as if it were evidence, so opening /banzai reported a
# technical number before anything had been validated. ADR-076 removed that class of bug at the root:
...
# The guidance layer has no blocker/evidence verdict vocabulary (ADR-076 §D-076-02), so there is no
...
# M2.19G.1 (ADR-068) — the validation surface (BanzaiValidationMode.tsx) shows Progresso (n/total steps)
