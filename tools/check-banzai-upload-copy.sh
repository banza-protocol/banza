#!/usr/bin/env bash
# make banzai-upload-copy-check — M2.9C guard (ADR-036), retargeted for M2.19G.1 (ADR-034).
#
# Endpoint-originated model (ADR-034 §4.4/§4.5): the OFFICIAL operator-validation journey takes NO manual
...
# M2.19G.1 (ADR-034) — upload/paste live ONLY in the isolated developer draft tool.
...
echo "== banzai-upload-copy-check (M2.9C / ADR-034) =="
...
# 2. Upload/paste live ONLY in the isolated developer draft tool (ADR-034 §4.5), never the official path.
if [ -f "$DRAFT" ]; then
...
for f in "$VJOURNEY" "$VSHELL"; do
  ...
  if grep -qE 'type="file"|<textarea|scanUpload\(|DRAFT_VALIDATION_RESULT' "$f"; then
    fail "$(basename "$f") exposes a manual-input affordance — the official journey must be endpoint-originated (ADR-034 §4.4)"
  else
...
# 7. Isolation (ADR-034 §4.5): the draft tool forwards the raw file body NOWHERE — it never sends it to
