#!/usr/bin/env bash
#
# banza-certification-model-check (M2.19D / ADR-032/065/066).
...
#  - the closed certification-state machine is exactly the ADR-032 enum (fail-closed; REVOKED terminal);
...
for f in \
  $(ls decisions/adr/ADR-032-*.md 2>/dev/null | head -1) \
  $(ls decisions/adr/ADR-033-*.md 2>/dev/null | head -1) \
  $(ls decisions/adr/ADR-032-*.md 2>/dev/null | head -1) \
...
# 3. closed state machine — exactly the ADR-032 enum, in the Rust engine
