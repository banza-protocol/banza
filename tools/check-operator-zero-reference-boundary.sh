...
# "first"/"official" operator, an authority, or a disguised normative specification. The canon (ADR-067
# read-only canonical reference implementation · ADR-052/053 demo policy · ADR-068 endpoint-originated
# validation · ADR-060/061 separation) fixes it as a non-normative, non-production, no-real-funds,
...
echo "Operador Zero reference-boundary guard — §9 keeps a reference implementation from drifting into a privileged/normative/production operator (ADR-067/052/053/068)"
if check "$REF"; then
  ...
  echo "Result: ✗ §9 lets Operador Zero drift into a privileged/normative/production operator (see decisions/adr/ADR-067, ADR-052)"
  exit 1
fi
