#!/usr/bin/env bash
#
# BanzAI single canonical interface guard (M2.19E/F.2, ADR-067).
#
...
# Like the other archival ADRs (ADR-044/045/046 describe the earlier `mock` default), it is rendered
...
ARCHIVAL_ADR="ADR-067-operador-zero-read-only-reference-and-banzai-validation-workbench.md"
...
# ── 2. Single BanzAI interface: only the ADR-070 closed navigable-context segments ────────────────────
# ADR-070 refines "single interface" from "one route file" to "one shell/app with one always-mounted
...
if [ -n "$BAD_TOP_DIRS" ]; then
  bad "$BANZAI_DIR/ has a non-allowlisted route subfolder (ADR-070 allows only 'operador/'):"
  printf '%s\n' "$BAD_TOP_DIRS" | sed 's/^/         /'
else
...
# If the operador segment exists, its nested shape must be exactly the ADR-070 closed tree.
if [ -d "$BANZAI_DIR/operador" ]; then
  ...
  if [ -n "$BAD_SEG" ]; then
    bad "$BANZAI_DIR/operador/ has non-allowlisted segments (ADR-070 allows only [operatorId]/[implementationId]):"
    printf '%s\n' "$BAD_SEG" | sed 's/^/         /'
  else
    ok "operador/ segment tree is the closed ADR-070 shape ([operatorId]/[implementationId])"
  fi
...
# ── 2b. ADR-070 D-070-05: contexts are NOT the 3 layers and NOT L0-L4 certification tiers ─────────────
...
if [ -n "$CONFLATE" ]; then
  bad "ADR-070 D-070-05 conflation: navigation context described as 'camada/layer' or L0-L4 as certification tiers:"
  printf '%s\n' "$CONFLATE" | sed 's/^/         /' | cut -c1-200
else
  ok "no context↔layer / L0-L4↔tier conflation on the /banzai surface (ADR-070 D-070-05)"
fi
...
if [ -n "$BW_HITS" ]; then
  ...
  ok "no 'BanzAI Web' brand in website active source, docs/**, decisions/** (excl. archival ADR-067), or SVGs"
fi
