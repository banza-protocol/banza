#!/usr/bin/env bash
# banzai-operator-zero-only-ui-check (ADR-035, M2.14B) — Part 17, retargeted for M2.19G.1 (ADR-034).
#
# The BanzAI product UI presents Operador Zero as the ONLY demo operator. In the endpoint-originated
# model (ADR-034), the OZ-only demo framing lives in Fase 0 of the validation mode — "Operador
...
echo "== banzai-operator-zero-only-ui-check (M2.14B / ADR-034) =="
