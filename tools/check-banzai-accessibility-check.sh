...
# 1. Exactly one h1 on the BanzAI surface. M2.19G.4 (ADR-070): the single sr-only <h1> now lives in the
...
[ -f "$LAYOUT" ] && grep -qE '<h1' "$LAYOUT" && ok "the single <h1> lives in the shared app/banzai/layout.tsx" || fl "$LAYOUT must carry the single BanzAI <h1> (ADR-070 shared layout)"
