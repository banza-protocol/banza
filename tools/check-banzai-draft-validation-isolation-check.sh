#!/usr/bin/env bash
#
# M2.19G.1 (ADR-068 §4.5/§17) — developer draft tool isolation guard (§37, invariant 5).
#
...
echo "== banzai-draft-validation-isolation-check (M2.19G.1 / ADR-068 §4.5/§17) =="
...
# 6. M2.19G.5C (ADR-074) — complementary to the SimB retirement: SimB survives ONLY inside the isolated
...
for f in website/components/banzai/banzai-agent.ts website/components/banzai/validationJourney.tsx; do
  [ -f "$f" ] || continue
  if grep -qE 'SimB|banza-simb' "$f"; then fl "SimB leaked onto an active surface: $f (ADR-074)"; else ok "$f is SimB-free (ADR-074)"; fi
done

echo
if [ "$fail" -ne 0 ]; then echo "banzai-draft-validation-isolation-check: FAIL"; exit 1; fi
echo "banzai-draft-validation-isolation-check: ✓ draft tool isolated, non-authoritative (ADR-068 §4.5/§17)"
