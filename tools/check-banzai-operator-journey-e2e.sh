...
# M2.19G.1 (ADR-068) — each step stores the SERVER-built OperationReceipt (endpoint-originated: the Rust
...
# verdict (ADR-076 §D-076-02 — it is guidance only), so there is no `toneVerdict` in the guidance
...
if grep -q 'export function toneVerdict' "$WRAPPER"; then
  bad "$WRAPPER must not map a tone to a verdict — Model A is guidance only (ADR-076 §D-076-02)"
else
...
// A referenced Model B FAILED verdict never becomes a positive navigation state (ADR-076 §D-076-02).
