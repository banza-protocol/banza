#!/usr/bin/env bash
#
# BANZA Regulatory-State Claim Guard (M2.19C, ADR-007 / ADR-006).
#
...
# The prudent phrasing and the state name are canonical in the claim policy and the scheme document;
# the record explains why the gate exists and is not a second home for the wording (ADR-010).
need  "$ADR062" 'preparation state'                   'ADR-007 states the preparation state grants nothing'
need  "$RCP"    'em preparação regulatória'            'claim policy carries the prudent phrasing'
...
if [ "$fail" -ne 0 ]; then
  echo
  echo "regulatory-state-claim: FAIL — see ADR-007 and docs/governance/BANZA_REGULATORY_CLAIM_POLICY.md."
  exit 1
fi
echo
echo "regulatory-state-claim: ✓ prudent phrasing + REGULATORY_AUTHORIZATION_IN_PROGRESS canonical; no already-authorised/BNA-approved/licensed claim (M2.19C / ADR-007)"
