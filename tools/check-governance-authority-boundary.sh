...
# ADR-010 current-only ADR tree · ADR-025 Model A · ADR-005/066 the verdict/admission owners) fixes
# governance as:
#
...
echo "Governance authority-boundary guard — §11 keeps governance a public process of the maintainers over the RULES (GOVERNANCE.md · ADR-010/061/066/079), never a regulator/administrator of operators and never the Trust Root governing the protocol"
if check "$REF"; then
  ...
  echo "Result: ✗ §11 lets governance drift into a regulator/administrator of operators, or the Trust Root into the government (see GOVERNANCE.md, decisions/adr/ADR-010, ADR-005, ADR-032, ADR-025)"
  exit 1
fi
