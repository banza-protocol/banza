...
#
# The BANZA PostgreSQL is a verifiable protocol-state store, not a financial database. Per ADR-042 it
...
for f in $(git ls-files $SCHEMA_GLOB 2>/dev/null | grep -viE '(^|/)[^/]*test[^/]*\.sql$' || true); do
  ...
  if [ -n "$hits" ]; then
    echo "NEEDS_FIX  forbidden financial/personal/secret identifier in protocol schema (ADR-042 — the BANZA database stores protocol state, not financial value):"
    echo "$hits" | sed 's/^/    /'
    fail=1
  fi
done
