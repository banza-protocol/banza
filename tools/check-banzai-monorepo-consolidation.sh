#!/usr/bin/env bash
# check-banzai-monorepo-consolidation.sh — M2.19G.6 (ADR-036) consolidation invariants.
#
...
#   4. ADR-036 exists and the repo-guards ADR range admits it;
...
echo "== banzai-monorepo-consolidation-check (M2.19G.6, ADR-036) =="
...
# 4. ADR-036 exists.
[ -f $(ls decisions/adr/ADR-036-*.md 2>/dev/null | head -1) ] \
  && ok "ADR-036 present" || fail "ADR-036 missing"
...
#    permitted whole-token mention in an active index is inside the removal decision records (ADR-036 and
#    the ADR-036 it supersedes), which describe the repo as *removed* and carry no URL.
node - <<'NODE'
const fs=require("fs");
...
const ALLOW=/ADR-036|ADR-036/;                         // removal decision + what it supersedes
const HIST=[];
