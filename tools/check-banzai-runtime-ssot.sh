#!/usr/bin/env bash
#
# BanzAI runtime SSOT guard (M2.19G.5C, ADR-072).
#
# ADR-072 introduces ONE public runtime single-source-of-truth: GET /banzai/runtime — a secret-free,
...
if [ "$fail" -ne 0 ]; then
  echo
  echo "banzai-runtime-ssot: FAIL — the runtime SSOT contract (ADR-072) drifted."
  exit 1
fi
echo
echo "banzai-runtime-ssot: ✓ one secret-free /banzai/runtime SSOT (schema_version + authoritative:false); /estado consumes it and never hardcodes the live engine (ADR-072)"
