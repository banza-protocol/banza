...
#   2. the Rust-first legacy allowlist (ADR-037) — the named set of not-yet-migrated non-Rust code.
#
...
lg_allow  "the ADR-037 legacy allowlist"   'README.md:2:UI/glue, Python is temporary legacy. A CI guard blocks new non-Rust engines.'
...
# The two live uses (the /certificates route, the ADR-037 legacy allowlist) clear via legacy_filter,
...
if [ -n "$legacy" ]; then
  echo "NEEDS_FIX  'legacy' on a product surface. It may name the /certificates compatibility route, or the Rust-first legacy allowlist (ADR-037) on a line that carries no trust vocabulary — nothing else:"
  echo "$legacy" | sed 's/^/    /'
  fail=1
fi
...
# decisions, byte-copied from decisions/adr|rfc); technical ADRs legitimately use "corpus" (e.g. ADR-055/056
