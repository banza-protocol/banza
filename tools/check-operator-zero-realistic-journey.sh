#!/usr/bin/env bash
#
# operator-zero-realistic-journey-check (ADR-035, M2.14A).
#
...
# M2.19G.4 (ADR-036) — the shell now consumes the server-resolved route state as `routeState` (renamed
...
# M2.19G.1 (ADR-034) — the endpoint-originated session dispatches each step via runOne, which calls the
...
# M2.19G.1 (ADR-034): each step carries its own evidence references (evidence_refs, taken from the
