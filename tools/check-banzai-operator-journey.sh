#!/usr/bin/env bash
# make banzai-operator-journey-check — M2.9B guard (ADR-036).
#
...
#   - the guided layer stays GUIDANCE ONLY (ADR-036): navigation statuses, no verdict/score,
...
# M2.19G.1 (ADR-034) — the endpoint-originated validator: the browser calls this backend, which fetches
...
if [ -f "$ENGINE" ]; then
  ...
  # ADR-036 — Model A is guidance only; Model B is the single technical-state authority.
  grep -qiE 'guidance only|orienta o percurso' "$ENGINE" \
    ...
    || fail "$ENGINE must state Model A is guidance only — Model B is the single technical-state authority (ADR-036)"
else
...
if [ -f "$AGENT" ]; then
  ...
  # M2.19G.1 (ADR-034 § Recursos) — the Repositório link moved out of the primary nav into Programadores.
...
# M2.19G.1 (ADR-034) — the validation journey is ENDPOINT-ORIGINATED and engine-driven: the browser calls
