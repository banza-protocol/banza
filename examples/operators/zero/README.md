...
**Implementação de referência canónica, só-leitura, do protocolo BANZA (ADR-035).**
...
Decisão actual: [ADR-035](../../../decisions/adr/ADR-035-operator-zero-the-read-only-reference-implementation.md)
(implementação de referência só-leitura + validação no BanzAI). O ADR-035 **substitui o enquadramento
anterior** do
[ADR-035](../../../decisions/adr/ADR-035-operator-zero-the-read-only-reference-implementation.md) — cujo nome
de ficheiro conserva "simulator" apenas como identificador arquival estável — e do
[ADR-035](../../../decisions/adr/ADR-035-operator-zero-the-read-only-reference-implementation.md) (decisões anteriores).
...
primária e transversal (ADR-036). As nove etapas — Discovery, Manifest, Keys, Conformance, Interoperability,
...
`404` JSON. Sem HTML nas rotas de máquina, sem segredos, sem PII (ADR-035).
...
Este directório de exemplo antecede o ADR-035 e retém material demonstrativo anterior da fase ADR-035. O
modelo canónico actual é o do ADR-035: superfície **só-leitura**, sem ledger mutável e sem simulação. A
definição canónica vive na Referência (`/referencia/operador-zero`) e no ADR-035.
