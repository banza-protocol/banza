...
- **Governing decision:** [ADR-036](../../decisions/adr/ADR-036-banzai-a-non-authoritative-interface-to-the-protocol.md) (agent core / journey), [ADR-035](../../decisions/adr/ADR-035-operator-zero-the-read-only-reference-implementation.md) (nine-step validation journey), [ADR-034](../../decisions/adr/ADR-034-endpoint-originated-validation.md) (endpoint-originated validation + operator/implementation model)
...
> **Endpoint-originated (ADR-034, M2.19G.1).** The **official** validation journey uses **exclusively
...
> (`DRAFT_VALIDATION_RESULT`, ADR-034 §4.5); it never enters the official journey. See the
> [Reference — BanzAI chapter](../../docs/reference/pt/BANZA_REFERENCIA.md) and ADR-034 §19, which
...
All journey logic is computed in Rust (rules 15/16 of ADR-036), never in TS:
...
## 3. JSON upload — the local draft tool (M2.9C; draft-only since ADR-034)
...
evidence** (`DRAFT_VALIDATION_RESULT`: local, non-authoritative, never evidence — ADR-034 §4.5). An
...
certification is not scheme admission nor regulatory authorisation** (ADR-005). The official journey is
endpoint-originated (ADR-034): its verdicts are decided in Rust over content fetched from the
