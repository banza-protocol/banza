...
> **BanzAI is the native, non-authoritative AI agent of the BANZA protocol** (ADR-036). It guides
...
- **Official validation is endpoint-originated (ADR-034).** The nine-step journey validates a published
  ...
  **draft** tool only. See [OPERATOR_JOURNEY.md](OPERATOR_JOURNEY.md) and
  ADR-034 (§19 for the SSRF policy), which `engines/banza-artifact-fetcher` implements.
...
ADR-036 (native protocol agent) · ADR-036 (local Qwen runtime + benchmark gate) · ADR-036 (latency
tuning) · ADR-036 (reasoning disabled + warm-up) · ADR-036 (384-token default) · ADR-036 (Qwen-first
routing) · ADR-036 (protocol-agent core, journey, session) · ADR-036 (unified `/banzai` interface) ·
ADR-036 (per-answer execution metadata) · ADR-035 (Operador Zero read-only reference + nine-step
validation journey) · ADR-034 (endpoint-originated operator validation + operator/implementation model).
