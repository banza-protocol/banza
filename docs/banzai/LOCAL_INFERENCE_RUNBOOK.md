...
on-host `local_qwen` language layer for BanzAI (ADR-044, latency-tuned per ADR-045).
...
# --- generation limits (tuned for CPU inference — ADR-045) ---
LLM_TIMEOUT_MS=60000             # local_qwen default (60s operational margin); 90000 only as a documented extreme fallback
LLM_MAX_TOKENS=384               # local_qwen default (ADR-047; professional answers); 256/512 only by explicit config; hosted providers keep 800
...
### Tuning output length and timeout (ADR-045)

`local_qwen` ships CPU-tuned defaults: `LLM_MAX_TOKENS=384` (ADR-047) and `LLM_TIMEOUT_MS=60000`
(60s). Hosted providers keep their own default (800 tokens). Tune within these bounds:

- **`LLM_MAX_TOKENS`** — 384 is the local default (ADR-047: a professional answer budget; the VPS
...
### Re-benchmark after tuning (ADR-045)
...
**Reasoning disabled (ADR-046).** For the local Qwen3 runtime, BanzAI disables the model's thinking
...
- `sources_count` — cited sources per answer; **≤3** for local inference (ADR-045 source
...
model choice are revised (ADR-044 §7). Roll back per §9.

---

**See also:** ADR-044 (BanzAI Local Qwen Inference Runtime), ADR-045 (BanzAI Local Qwen
Latency Tuning), ADR-037 (Rust-first engines), ADR-041 (BanzAI as native protocol agent),
