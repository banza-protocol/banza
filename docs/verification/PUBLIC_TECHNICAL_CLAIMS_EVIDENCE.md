...
- §6 «todos os passos técnicos estão verificados» — readiness is correctly **profile-scoped** in the
  engine (ADR-077 applicability); the prose could add «aplicáveis ao perfil declarado». Optional precision.
...
executed**: the federation OTE now performs all ten ADR-040 checks and emits `ROUTING_ALLOWED` /
