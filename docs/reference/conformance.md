...
BANZA is an open financial protocol. Operator participation is demonstrated by verifiable protocol conformance, not by central human approval. The suite produces evidence; it produces no decision, and nobody reviews the result on your behalf.
...
| L3 | Inter-Operator Interoperability | All L2 + federation routing, reconciliation, inter-operator settlement, signed protocol metadata verifiable to the trust root |
...
ADR-030. The sandbox runner covers L0–L2; L3 (federation) and L4 (external) are
not covered by it.

The reference operator **targets Level 2** and can be evaluated against the
Level-2 suite. No conformance claim is asserted here — such a claim requires
committed conformance evidence (a `report.json` with provenance), which this
repository does not include.

Passing the suite makes an operator **evaluable**, not routable. Routing is decided
by each counterparty independently, via Open Trust Evaluation (ADR-025), of which
...
mark) — counterparties verify your signed evidence, never a badge.
...
## CI integration (maintainer/internal tooling)

The following is the BANZA repository's own continuous-integration setup, used by protocol **maintainers** — it is not an operator step. Operators validate conformance in the BanzAI. The repository's `conformance` CI runs on every push to `main` and every pull request:
