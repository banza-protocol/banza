...
│   ├── banzai-authority-chain-v1.svg        SVG-P-021 (RETIRED — asset absent; superseded by ADR-036)
...
BanzAI SVGs are consolidated into this monorepo (ADR-036) and live at `website/public/diagrams/banzai/` and `docs/reference/diagrams/protocol/banzai-*.svg` — there is no separate BanzAI repository. They must still comply with brand and language standards.
...
Diagramas do modelo de trust aberto (ADR-025 · ADR-031 · ADR-025). A participação de operadores
...
| ~~SVG-P-079~~ | ~~`operador-zero-architecture-v1.svg`~~ | **RETIRADO (M2.19G §29)** | Arquitectura do simulador (ledger fictício, arquitectura do simulador) — ficheiro removido; substituído pelo conjunto v2 SVG-P-088/089 (alvo de validação só de leitura + separação de responsabilidades). ADR-035. |
| ~~SVG-P-080~~ | ~~`operador-zero-e2e-journey-v1.svg`~~ | **RETIRADO (M2.19G §29)** | Jornada E2E de seis passos com score 100/100 — ficheiro removido; substituído por SVG-P-090 (jornada de validação de nove passos, sem score). ADR-035. |
| ~~SVG-P-081~~ | ~~`operador-zero-negative-flow-v1.svg`~~ | **RETIRADO (M2.19G §29)** | Fluxo negativo auto-executado — ficheiro removido; a referência só de leitura não executa simulação (ADR-035). |
| ~~SVG-P-082~~ | ~~`operador-zero-ledger-kzdemo-v1.svg`~~ | **RETIRADO (M2.19G §29)** | Ledger fictício KZ_DEMO — ficheiro removido; a referência só de leitura não corre ledger mutável (ADR-035). |
...
ao modelo activo M2.19 (ADR-004..067): três camadas **Camada 1 Protocolo aberto · Camada 2 Certificação de Conformidade e
...
fechada (ADR-032); e a separação **certificação técnica ≠ admissão a esquema ≠ autorização regulatória**
(ADR-005). Nenhum diagrama apresenta conceitos retirados: autoridade central de emissão, credencial por
...
## M2.19G.1 §35 — Validação originada nos endpoints (ADR-034)
...
- **`banzai-single-answer-pipeline.svg` (SVG-P-087)** — o embed foi removido de `BANZA_REFERENCIA.md` §12
  ...
  segundo validador obrigatório (validador de pós-resposta, ADR-036) ao tronco explicativo. A prosa de §12
