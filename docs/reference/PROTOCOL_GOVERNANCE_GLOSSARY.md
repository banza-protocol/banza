...
| **invariant** | A non-negotiable integrity rule the protocol guarantees — families `INV-LEDGER`, `INV-WALLET`, `INV-SETTLE`, `INV-IDEM`, `INV-RECON`, `INV-QR`. | An integrity guarantee; never weakened for convenience. | `decisions/adr/ADR-012-*`, invariants registry |
