...
**Authority:** ADR-001, ADR-025, ADR-031, ADR-025, ADR-030
...
- Signed protocol metadata self-published at `/.well-known/banza/protocol-metadata.json` (ADR-031)
...
5. **Sign and publish your protocol metadata.** Reference the evidence by hash at `/.well-known/banza/protocol-metadata.json`, signed and anchored in the protocol trust chain (ADR-031). Nobody issues it for you.
...
Before any payment, Operator A runs the [Open Trust Evaluation](#open-trust-evaluation) over Operator B (ADR-025). This evaluation is cryptographic and local — it does not require a real-time call to BANZA and consults no authority.
...
| Item | Status |
|------|--------|
| Architecture specification (ADR-025 + ADR-025) | COMPLETE |
...
is **protocol-owned**, not SDK-owned (full specification: ADR-025):
...
- **Versioning.** Manifests are versioned and supersede one another; a historical
  ...
  signed under earlier manifests (ADR-025).
...
Each operator publishes and signs its own protocol metadata — nobody issues it on its behalf (ADR-031). The metadata is served on the operator's own domain at:
...
Key Manifest (see *Trust anchor distribution*, ADR-025) — SDK bundling is a
...
**ADRs (Architecture Decision Records)** record decisions after they are made: technology choices, service boundaries, SDK architecture, naming. ADRs are numbered sequentially and immutable after acceptance. ADR-001 defines the canonical BANZA/BanzAI/Operators hierarchy. ADR-025 defines the open protocol trust model, including the offline Trust Root and its delegated-key hierarchy. ADR-031 defines operator self-publication and machine-verifiable conformance. ADR-025 defines federation trust evaluation.
...
|-----------|:--------:|---------|
| M1 — Protocol Complete | **2026-06-01** | federation and interoperability conformance test set verified; the trust and federation model was later redesigned in M2.3 (ADR-025/ADR-031/ADR-025) |
