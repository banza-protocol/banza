...
**Autoridade:** ADR-001, ADR-025, ADR-031, ADR-025, ADR-030
...
Para a arquitectura do BanzAI, consulte **[Referência §12 — BanzAI](/referencia/banzai)** (fonte canónica). O desenvolvimento activo do BanzAI reside inteiramente neste repositório (`services/banzai-api` + `engines/banzai-*`); não existe um repositório BanzAI separado (ADR-036).
...
| **BanzAI** | Interface humana primária e transversal e motor cognitivo não autoritativo do protocolo (ADR-036, ADR-004) — guia, invoca os motores verificáveis e explica; não normativo, não decide, não certifica | Disponível |
...
**Na prática:** A Trust Root BANZA é controlada por **três autoridades de assinatura independentes**, e qualquer acção autorizada da raiz exige **duas assinaturas de duas delas**. Nenhuma autoridade isolada autoriza. O limiar é criptográfico; o número de dispositivos é um controlo de custódia e não o define.
...
- Identidade @handle com unicidade global — um identificador de carteira no formato `@nome` que identifica univocamente um titular na rede BANZA, independentemente do operador onde está registado (ADR-014)
...
Cada operador publica e assina a sua própria metadata de protocolo — ninguém a emite em seu nome (ADR-031). A metadata é disponibilizada no domínio do próprio operador em:
...
A Trust Root é gerada numa **cerimónia offline** numa máquina isolada da rede, sem conectividade, na presença de uma testemunha independente. Material de chave privada nunca toca numa máquina ligada à rede. A raiz é controlada por **três autoridades de assinatura independentes**, cada uma com uma chave na sua própria máquina offline e a sua própria cópia cifrada no seu próprio local. Uma acção autorizada da raiz exige **duas das três**: nenhuma autoridade isolada reconstrói ou activa a raiz, e perder uma não a bloqueia.
...
O protocolo utiliza ed25519 como mecanismo de assinatura para todos os artefactos da cadeia de confiança (ADR-025). Esta escolha é documentada e auditável.
...
| Componente | Estado |
|-----------|--------|
| Arquitectura da Trust Root (ADR-025) | CONCLUÍDA |
...
|---|---|
| Autorizar, admitir ou aprovar um operador | Não existe função de admissão no protocolo — ADR-025 |
| Emitir qualquer artefacto de estatuto a favor de um operador | O operador auto-publica a sua própria metadata — ADR-031 |
...
Antes de qualquer pagamento, o Operador A executa a Avaliação Aberta de Confiança sobre o Operador B ([§8](#avaliação-aberta-de-confiança), ADR-025). Esta avaliação é criptográfica e local — não requer uma consulta em linha ao BANZA e não consulta nenhuma autoridade.
...
| Item | Estado |
|------|--------|
| Especificação de arquitectura (ADR-025) | CONCLUÍDO |
...
A governação evolui as regras do protocolo. Não admite, aprova, autoriza nem certifica operadores — essa função não existe no protocolo (ADR-025).
...
**N3 — ADRs** concretizam os princípios em decisões de arquitectura: o livro-razão de partidas dobradas (ADR-012), a idempotência obrigatória (ADR-022), o modelo de confiança aberto sem autoridade central, incluindo a trust root offline e as chaves delegadas (ADR-025), a auto-publicação e conformidade verificável por máquina (ADR-031), a avaliação de confiança de federação (ADR-025). As ADRs são imutáveis após aceitação. Uma ADR que contradiga um invariante é inválida por definição. Uma ADR pode ser substituída por outra posterior, com rasto explícito.
...
| ADR | Decisão |
|-----|---------|
| ADR-012 | Livro-razão de partidas dobradas como base financeira |
| ADR-022 | Idempotência obrigatória e limitação de taxa |
| ADR-015 | Sistema de pagamento QR (estático e dinâmico) |
| ADR-014 | Modelo de identidade de conta/participante (com @handle) |
| ADR-001 | Protocolo financeiro aberto — independência de implementação |
| ADR-001 | Separação entre operadores e protocolo |
| ADR-001 | Hierarquia canónica BANZA/BanzAI/Operadores |
...
A imutabilidade dos ADRs após aceitação não é rigidez — é a base da confiança dos operadores. Um operador que implementa L3 contra ADR-012 precisa de saber que ADR-012 não vai ser silenciosamente alterado. Se uma regra mudar, muda através de um novo ADR, com número próprio, referências explícitas ao ADR que revoga, e o mesmo processo de avaliação pública. Nada muda sem rasto.
...
- **Definição de âmbito** — o que cada chave delegada pode assinar está definido no protocolo (ADR-025) e limitado por `INV-ROOT-008`; a operação não define o seu próprio âmbito
...
|---|---|---|
| Supervisão da operação de assinatura | Define o âmbito delegado, audita práticas, pode substituir os custódios | ADR-025 |
...
> de nove passos (ADR-035/068), síntese única do modelo local (ADR-036),
> verificação final obrigatória em Rust (ADR-036) e estado verificável em runtime
> (ADR-036) — é mantida como fonte única em
...
|-------|:---------:|---------|
| M1 — Especificação Completa (v1.0) | **2026-06-01** | conjunto de testes de federação e cenários de interoperabilidade verificados; o modelo de confiança e federação foi redesenhado em M2.3 (ADR-025/ADR-031/ADR-025) |
...
Ninguém. Não existe autoridade certificadora, entidade de admissão nem processo de aprovação no protocolo (ADR-025). BANZA é um protocolo financeiro aberto. A participação de operadores é demonstrada por conformidade protocolar verificável, não por aprovação humana central. Ver [§7 Conformidade e Evidência](#7-conformidade-e-evidência).
...
- ADR-031 — Auto-publicação do operador e conformidade verificável por máquina
- ADR-025 — Avaliação de confiança de federação sem certificados
- ADR-013 — PostgreSQL como base de estado protocolar (não livro-razão financeiro)
