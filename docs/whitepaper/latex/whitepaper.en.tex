...
Design decisions follow four principles --- \textbf{BANZA R\textsuperscript{2}S\textsuperscript{2}}: \emph{robust}, correct and deterministic under independent implementation and adversarial input; \emph{resilient}, failures contained and recovery deterministic without weakening guarantees; \emph{secure}, critical properties enforced by construction, failing closed; \emph{simple}, the smallest sufficient mechanism. Resilience does not override security, nor promise continuous availability. They are design criteria, distinct from the invariants that follow. The architecture in turn rests on five architectural invariants: \emph{open specification} --- the applicable rules, contracts and profiles are public and versioned; \emph{implementation independence} --- no particular implementation constitutes the protocol, and the reference implementation realises it without defining it; \emph{independent verification} --- the published artifacts must suffice for a third party to assess the applicable conditions without depending on a private decision; \emph{operational independence} --- no central infrastructure is required to carry messages or funds; and \emph{separation of decisions} --- conformance evaluation, technical certification, admission to a scheme, operational agreements and regulatory authorisation are distinct processes.
...
Identity and integrity rest on published keys and signed metadata. The trust root is not a key: it is a set of three independent authorities, kept offline, where any authorised action requires two of them. The set advances as a lineage — the genesis set is accepted only against a digest the verifier was explicitly given, and every later set is authorised by the threshold of the set it succeeds, naming it by digest. A set signed by its own keys authorises nothing. Continuity follows: if an authority is lost, compromised or obstructive, the remaining two replace it without its participation; below the threshold, canonical continuity is blocked, with no single-party recovery key. The active set signs the Key Manifest with Ed25519 [6]; that manifest authorises delegated keys separated by domain, which sign the corresponding artifacts. Revocation and validity control reject revoked or expired material.
