...
BANZA's guarantees have clear boundaries. They are technical and not regulatory: a positive result demonstrates conformance with a public profile, without implying authorisation to operate, admission to a scheme or a commercial agreement.
...
BANZA remains in pre-production: the Technical Registry contains zero production operators and zero active certifications, payments with real money are disabled and the empty registry is the expected state. The reference implementation, Operator Zero, runs in an isolated test environment with demonstration currency, does not move real funds, remains uncertified and is evaluated by the same contracts and the same journey as any other implementation.
...
The completeness of a distribution and the capability for independent implementation are distinct properties: the first was verified, the second was not, and it remains to be demonstrated through a third-party implementation built without recourse to the reference code. Demonstrating it will require an external implementation built from the public specifications, able to satisfy the applicable vectors and to take part in a directional evaluation with another implementation, producing semantically equivalent results and verifiable evidence.
