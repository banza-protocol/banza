...
| R-TRUST-001 | trust/key management | Wrong signing-key domain used (root vs cert vs revocation) | trust bypass | LOW | HIGH | domain-separated keys (ADR-027); trust engine form checks | banza-trust; ceremony docs | root ceremony is M2 (pending) | keep domain separation; M2 root ceremony under two-of-three authorization | protocol governance | mitigated | trust |
