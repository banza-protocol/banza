...
| Settlement inconsistency | `net = gross − fee` coherence, ≥ 0, linked to intent (ADR-019/ADR-040) | protocol |
...
> **M2.19C introduces the three-layer institutional architecture** (ADR-059..063): L1 the open BANZA
> protocol, L2 per-implementation conformance & interoperability certification, L3 the designated L3
> scheme operator's operational scheme (ADR-060), conditioned on a regulatory framework. The layers are
...
- **certification vs scheme admission vs regulatory authorisation** — three separate determinations
  (ADR-061). A certified implementation is not thereby admitted to any scheme and not thereby authorised
  to move real funds.
- **BANZA (L1/L2) vs the designated L3 scheme operator** — the protocol and certification are
  operator-neutral (ADR-059/060); naming the first scheme operator does not make BANZA an operator, and
  the operator's own implementation earns no self-privilege (ADR-063).
...
> public endpoints of the selected implementation** (ADR-068). The target is resolved from the closed
> Technical Registry (`operator_id → implementation_id → canonical_origin → discovery`, ADR-065); the
> fetch is performed by a secure, SSRF-hardened **Rust** artifact fetcher (`engines/banza-artifact-fetcher`,
> ADR-037/ADR-068 §4.7), never the browser; the no-network decision engines receive already-fetched
...
> **draft** tool (`DRAFT_VALIDATION_RESULT`, ADR-068 §4.5). This section **appends** the threats this
...
| Target substitution — a caller tries to point validation at a different operator/implementation or a foreign origin | The target is resolved **only** from the closed Technical Registry (ADR-065); `operator_id`/`implementation_id` are closed-set ids (never URLs), and the fetcher receives a registry-derived `canonical_origin`+`expected_host`, never a caller URL | engine (registry) |
...
| Operador-Zero fixture bypass — the reference implementation is given a shortcut, official fixture or pre-computed result | Operador Zero receives **no** shortcut/fixture/bypass (ADR-068 §4.9): it exists in the registry as an operator + implementation record, publishes its endpoints at `zero.banza.network`, and is validated through the **same** secure fetch + Rust engines as any implementation, producing real origin-bound receipts | engine (fetcher/registry) |
