...
by the root or protocol-metadata-signing keys (ADR-025 domain separation).
...
- Operators MUST fetch a fresh BRL within its cache window (≤ 6 h, ADR-025); a stale BRL
...
4. **Sign (revocation-domain key).** Sign the BRL with the **BRL-issuing** key using canonical JSON
   (ADR-025). No other domain key may sign a BRL (INV-ROOT-005; the root itself signs only Key Manifests, INV-ROOT-004). *(Test-only in any rehearsal — no
