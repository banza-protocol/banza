...
governed by ADR-025 (open trust model — canonical-JSON signing, domain separation), ADR-025
(federation trust evaluation) and ADR-027 (keys never on serving infrastructure).
...
- ADR-025 and ADR-027 frozen and referenced (root offline, keys never on serving infra) — **met at spec level**.
...
Per [`KEY_MANAGEMENT_POLICY.md`](KEY_MANAGEMENT_POLICY.md) and ADR-027:
...
touches the runtime verification path (ADR-025): signed protocol metadata verifies against the issuing public key
...
Every signature uses canonical JSON (ADR-025). No key signs across domains (INV-ROOT-004). In M2 this
