...
> by **ADR-032** (certification model), **ADR-033** (Technical Registry) and **ADR-032** (closed
> certification-state machine), within the three-layer architecture of **ADR-004** and the separation of
...
- **`CertifiedImplementation`** — the subject, an *implementation* identified by the content hash of the
  ...
  is a different subject (ADR-032).
- **`InteroperabilityCertificationProfile`** — the public, versioned yardstick: the conformance level,
  ...
  the validity window, each pinned by hash and derived only from L1 protocol contracts (ADR-032).
- **`InteroperabilityCertificationRecord`** — the verdict: a `ConformanceReport` **and** an
  ...
  and a `record_hash` over the whole (ADR-032).
...
configuration may turn a FAIL into a PASS or widen a scope** (ADR-038, ADR-025 INV-OTE-008, ADR-032).
...
- It is **not a licence** and **not regulatory authorisation** — that is granted by the competent regulator;
  BANZA is not a party to it and does not grant, hold, accelerate or substitute for it (ADR-005,
  ADR-007).
- It is **not scheme admission** — admission is a separate, later determination made by a scheme (e.g. the
  ...
  admission never implies authorisation (ADR-005/05/06).
...
certification** — never as certified (ADR-032..07).

## The BANZA Technical Registry (ADR-033)
...
- ADR-032 (certification model), ADR-033 (Technical Registry), ADR-032 (state machine)
- ADR-004 (three-layer architecture), ADR-005 (certification ≠ admission ≠ authorisation), ADR-007
  (regulatory-state boundary), ADR-038 (Rust-first), ADR-025 (open trust model without a CA)
