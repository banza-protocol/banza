...
This policy is the canonical statement of the conflict-of-interest control fixed by **ADR-007** (with
ADR-003 D-059-06 and ADR-006 D-060-05). It states the risk, the controls, the prohibitions, how independent
...
Under ADR-044, **Banzami — Tecnologia e Serviços, Lda.** is the creator and initial institutional
maintainer of the open protocol. Under ADR-006, the same company is the **designated operator** of the first
...
- **Brand collapse.** A reader concluding "BANZA = Banzami = the scheme = authorised", losing both the
  protocol's neutrality and the regulatory boundary (ADR-006, ADR-005).
...
suspension/revocation** as any other implementation (ADR-007 D-063-02, ADR-006 D-060-05). Certification is
...
or a reason code (ADR-043/059). No human may convert a negative conformance result into a positive one
(ADR-027 INV-OTE-008). Rust validates before anything is published.
...
a privilege in another (ADR-007 D-063-03; `docs/governance/BANZA_SEPARATION_MATRIX.md`).

**C4 — Key separation.** The eight key domains are never reused across domains; the offline trust root and
its delegated signing keys never reside on serving infrastructure (ADR-007 D-063-04, ADR-029/038). Banzami's
...
Banzami account (ADR-006 D-060-06, ADR-007 D-063-06).
...
implementation is never suppressed, delayed or exempted (ADR-007 D-063-05, ADR-027 INV-OTE-005/006).
...
These prohibitions mirror ADR-006 D-060-05 and are made enforceable by ADR-007.
...
- Anyone can re-execute the **public vectors** against Banzami's artifacts and reproduce its
  `conformance_report_hash`; a result that does not reproduce is invalid (ADR-027 INV-OTE-004).
- Anyone can verify Banzami's certification record against **root-signed protocol metadata**, with **no
  Banzami credential, scheme membership or privileged endpoint** required (ADR-007 D-063-06).
...
- No BANZA-issued artifact about an operator is an input to the trust evaluation (ADR-027 INV-OTE-007), so
...
- **`banza-regulatory-state-claim-check`** — asserts no surface presents Banzami as already
  authorised/licensed/approved, keeping the regulatory-state boundary of ADR-005.
- **identity-guard** (`make identity-check`) — keeps payment-operator brands blocked everywhere and confines
  the Banzami name to the allowlisted institutional/scheme surfaces (ADR-044/059..063).
...
- ADR-007 (conflict of interest + infrastructure/key separation), ADR-006 D-060-05 (no self-privilege),
  ADR-003 D-059-06 (separation is an invariant), ADR-005 (regulatory-state + real-money gate)
- ADR-029 (keys never on serving infrastructure), ADR-043 (Rust-first engines), ADR-027 (open trust model
  without CA)
