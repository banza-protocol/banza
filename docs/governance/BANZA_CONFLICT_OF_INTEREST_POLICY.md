...
This policy is the canonical statement of the conflict-of-interest control fixed by **ADR-006** (with
ADR-004 and ADR-006). It states the risk, the controls, the prohibitions, how independent
...
Under ADR-009, **Banzami — Tecnologia e Serviços, Lda.** is the creator and initial institutional
...
- **Brand collapse.** A reader concluding "BANZA = Banzami = the scheme = authorised", losing both the
  protocol's neutrality and the regulatory boundary (ADR-006, ADR-007).
...
suspension/revocation** as any other implementation (ADR-006, ADR-006). Certification is
...
or a reason code (ADR-038/059). No human may convert a negative conformance result into a positive one
(ADR-025 INV-OTE-008). Rust validates before anything is published.
...
a privilege in another (ADR-006; `docs/governance/BANZA_SEPARATION_MATRIX.md`).

**C4 — Key separation.** The eight key domains are never reused across domains; the offline trust root and
its delegated signing keys never reside on serving infrastructure (ADR-006, ADR-027/038). Banzami's
...
Banzami account (ADR-006, ADR-006).
...
implementation is never suppressed, delayed or exempted (ADR-006, ADR-025 INV-OTE-005/006).
...
These prohibitions mirror ADR-006 and are made enforceable by ADR-006.
...
- Anyone can re-execute the **public vectors** against Banzami's artifacts and reproduce its
  `conformance_report_hash`; a result that does not reproduce is invalid (ADR-025 INV-OTE-004).
- Anyone can verify Banzami's certification record against **root-signed protocol metadata**, with **no
  Banzami credential, scheme membership or privileged endpoint** required (ADR-006).
...
- No BANZA-issued artifact about an operator is an input to the trust evaluation (ADR-025 INV-OTE-007), so
...
- **`banza-regulatory-state-claim-check`** — asserts no surface presents Banzami as already
  authorised/licensed/approved, keeping the regulatory-state boundary of ADR-007.
- **identity-guard** (`make identity-check`) — keeps payment-operator brands blocked everywhere and confines
  the Banzami name to the allowlisted institutional/scheme surfaces (ADR-009/059..063).
...
- ADR-006 (conflict of interest + infrastructure/key separation), ADR-006 (no self-privilege),
  ADR-004 (separation is an invariant), ADR-007 (regulatory-state + real-money gate)
- ADR-027 (keys never on serving infrastructure), ADR-038 (Rust-first engines), ADR-025 (open trust model
  without CA)
