...
`name`, `brl_url`, `conformance_url`, `contact`, `manifest_version`, `created_at`. (`certificates_url` é um campo legacy: continua a ser verificado como URL quando presente, mas já não é recomendado — ADR-029: não existe rota de índice de certificados.)
