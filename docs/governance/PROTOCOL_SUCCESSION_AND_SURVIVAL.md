...
- **Públicos.** Os vectores de conformidade (`conformance/`) e os motores de verificação (`engines/`, Rust por ADR-038) são abertos.
...
- **Motores oficiais em Rust** (ADR-038) — conformidade, verificação de confiança e da Revocation List, verificação de invariantes, geração de Evidence Bundle.
...
A Trust Root assina **apenas o Manifesto de Chaves**, que endossa as Delegated Signing Keys; são estas que assinam metadados do protocolo, releases e revogações (ADR-025).
...
- **Chaves fora da infraestrutura de serviço** (ADR-027). Material privado nunca vive em servidores de produção. Nenhum sistema em linha detém a capacidade de assinar como raiz.
...
- `decisions/adr/ADR-027-keys-never-on-serving-infrastructure.md` — chaves fora da infraestrutura de serviço
- `decisions/adr/ADR-038-rust-first-official-engines.md` — motores oficiais em Rust
- `decisions/adr/ADR-036-banzai-a-non-authoritative-interface-to-the-protocol.md` — BanzAI como agente nativo do protocolo
