...
| Blocker | Rev 1 | Rev 2 |
|---|---|---|
| X-01 Canonical byte form / signing envelope | **Missing (P0)** | **Supplied** — `spec/canonicalization.md` (`BCJ/1`) + 20 public vectors |
| X-02 Index of what constitutes BANZA v1.0 | Missing | **Supplied** — `contracts/production/normative-manifest.json`, 130 artifacts |
...
| X-04 Reason-code taxonomy | Missing | **Still missing** — deferred with rationale (F-05) |
| X-05 Idempotency rule | Missing | **Still missing** — deferred with rationale (F-06) |
...
|---|---|---|---|
| **M-00** | **Normative manifest** | `contracts/production/normative-manifest.json` | **What BANZA 1.0 *is*: 130 artifacts with SHA-256, precedence order on conflict, and an explicit statement of what is *not* normative.** This is the first document the team reads; every other line of this table is an entry in it |
...
| M-06 | Federation invariants | `spec/federation/FEDERATION_INVARIANTS.md` (63 refs) | INV-ROOT-*, INV-FEDEVAL-* |
...
| M-10 | OpenAPI surfaces (7) | `contracts/openapi/*.yaml` | REST surfaces incl. operator validation and interoperability certification |
...
| # | Artifact | Path | Note |
|---|---|---|---|
| **T-00** | **Canonicalization vectors (20)** | `conformance/vectors/canonicalization.json` | 14 accept + 6 reject, each with canonical form, byte length and SHA-256. The team validates its `BCJ/1` implementation before anything else |
...
validate it against 20 public vectors, then verify real root, delegation, metadata, manifest, evidence,
