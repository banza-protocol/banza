# ADR-026 — PostgreSQL as protocol state store, not a ledger

- **Status:** Accepted
- **Date:** 2026-07
- **See also:** ADR-011, ADR-011, ADR-008, ADR-008, ADR-027, ADR-033, ADR-031, `docs/governance/POSTGRESQL_PROTOCOL_STATE.md`, `infra/banza-network/postgres/init/001_schema.sql`
...
reference so the native protocol agent (BanzAI, ADR-042) can explain the protocol, and it keeps a
protocol audit log. ADR-008 already decided that this durable state lives in a **dedicated, internal
...
What ADR-008 did not state in normative form is **what PostgreSQL is allowed to represent**. Because
...
- The financial invariants (ADR-011/007, `INV-LEDGER-*`, `INV-WALLET-*`, `INV-SETTLE-*`) govern how an
...
2. **Least-privilege roles** (ADR-008) — `banza_ro` serves public routes read-only; `banza_gov`
...
- **A managed external database (Supabase/Firebase).** Rejected by ADR-008: it makes the protocol
...
- ADR-011 — Double-entry ledger; ADR-011 — Double-entry invariant enforcement
- ADR-008 — Dedicated independent infrastructure; ADR-008 — Dedicated PostgreSQL and encrypted backups
- ADR-027/039/040 — Open protocol trust model, operator self-publication, federation trust evaluation
- ADR-042 — BanzAI as native protocol agent
