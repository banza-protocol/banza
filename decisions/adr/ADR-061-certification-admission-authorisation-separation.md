...
- **Date:** 2026-07
- **Milestone:** M2.19C
- **Related:** ADR-059 (three-layer architecture), ADR-060 (Banzami Operational Scheme), ADR-038 (open
  trust model without CA), ADR-040 (federation trust evaluation without certificates), ADR-043 (licence &
  open governance — Banzami as creator/initial maintainer); companion: ADR-062 (regulatory-state boundary
  + RealMoneyActivationGate), ADR-063 (conflict-of-interest + infrastructure/key separation)

---

## Context

ADR-059 fixes the three-layer institutional architecture and ADR-060 names Banzami as the designated
...
no authority to grant. ADR-038/040 already removed any central authority over operators from the *trust*
...
discipline and is enforced by the M2.19C separation/regulatory-state guards; it is accepted because a
...
(ADR-001/003/059) all stand. This ADR separates the three *status* determinations; ADR-062 governs the
regulatory-state claims and the real-money gate that sit downstream of them.

## References

- ADR-059 (three-layer architecture), ADR-060 (Banzami Operational Scheme), ADR-062 (regulatory-state
  boundary + RealMoneyActivationGate), ADR-063 (conflict-of-interest + infrastructure/key separation)
- ADR-038/040 (open trust model / federation trust without certificates), ADR-043 (licence & open
  governance)
