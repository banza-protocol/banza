# ADR-023 — Reason codes

- **Status:** Accepted
- **Date:** 2026-08
- **Relates:** ADR-009 (versioning), ADR-042 (journey receipts), ADR-038 (endpoint-originated validation), ADR-031 (Open Trust Evaluation)
...
6. **Unknown core-shaped codes are preserved, not rejected.** Adding a core code is backward compatible
   under ADR-009; rejecting unknown codes would make every future addition a breaking change.
...
with what authority — those remain ADR-031, ADR-038 and ADR-042. It confers no certification meaning: a
