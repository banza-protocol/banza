...
- **Status:** Accepted
- **Date:** 2026-07
- **Companion:** ADR-038 — Open Protocol Trust Model Without CA (defines the Open Trust Evaluation this ADR applies to federation routing)
- **See also:** ADR-001, ADR-003, ADR-005, ADR-021, ADR-037, ADR-038, ADR-039, `docs/governance/OPEN_PROTOCOL_GOVERNANCE.md`, `docs/governance/OPEN_PROTOCOL_ARCHITECTURE.md`, `docs/governance/OPERATOR_SELF_PUBLICATION_AND_CONFORMANCE.md`

## Context

BANZA is an open financial protocol. Its trust model removes any central authority over operators: the
general Open Protocol Trust Model (ADR-038) replaced CA/certificate-based operator trust with signed
...
the Open Trust Evaluation (ADR-038), performed by the routing party itself over public material, with no
artifact issued by BANZA to an operator and no human step anywhere in the path.**

ADR-038 defines the Open Trust Evaluation as six ordered steps for the general trust model. For
federation routing this ADR decomposes that same evaluation into **ten concrete, independently-testable
checks** — the six steps of ADR-038 made explicit, with the protocol-version, capability, endpoint and
...
- **Does NOT prove:** that BANZA authorised, accepted or vouched for the operator. A trust root signature
  ...
  keys; protocol metadata, releases and revocations are signed by those delegated keys within their domains, never by the root directly (INV-ROOT-004; ADR-079). It does not authorise operators, does not authorise payments, does not create operators,
...
official engines are Rust (ADR-037), deterministic and cheap to run offline.
