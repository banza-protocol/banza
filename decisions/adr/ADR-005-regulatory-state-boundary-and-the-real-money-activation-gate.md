# ADR-005 — Regulatory-state boundary and the real-money activation gate

- **Status:** Accepted
- **Date:** 2026-07
- **Related:** ADR-003 (three-layer architecture), ADR-006 (Banzami Operational Scheme), ADR-004
  (certification ≠ admission ≠ authorisation), ADR-027 (open trust model without CA); companion: ADR-007
...
ADR-006 records that Banzami's internal state is `REGULATORY_AUTHORIZATION_IN_PROGRESS` and that every
...
ADR-003 D-059-05, Rust decides and Qwen never decides — so the gate is Rust and fail-closed, and no
...
Rust-sole-authority (ADR-043), the open trust model (ADR-027/040), and the certification ≠ admission ≠
authorisation separation (ADR-004) all stand. This ADR fixes the boundary on Banzami's regulatory-state
...
- ADR-003 (three-layer architecture), ADR-006 (Banzami Operational Scheme), ADR-004 (certification ≠
  admission ≠ authorisation), ADR-007 (conflict-of-interest + infrastructure/key separation)
- ADR-027 (open trust model without CA)
