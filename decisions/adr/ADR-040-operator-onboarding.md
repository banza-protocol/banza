# ADR-040 — Operator onboarding

- **Status:** Accepted
- **Date:** 2026-07
- **Related:** ADR-038 (endpoint-originated validation, operator≠implementation), ADR-041 (Operador
  Zero read-only reference), ADR-036 (Technical Registry), ADR-034/066 (L2 certification, closed
  state machine), ADR-042 (BanzAI as the single human-operator interface), ADR-026 (PostgreSQL as
  protocol-state store, not a financial DB), ADR-027 (open trust model, no central CA), ADR-043
  (Rust-first engines), ADR-008 (DNS & TLS), ADR-008 (dedicated PostgreSQL + backups)

---

## Context

ADR-038 established that BanzAI's official validation obtains every artifact from the public endpoints
...
journey of ADR-038 — there is no separate or privileged journey for candidates.
...
Persistence uses the existing dedicated PostgreSQL protocol-state store (ADR-026) with hash-only,
...
- New private onboarding tables in PostgreSQL (hash-named, within the ADR-026 boundary) via
