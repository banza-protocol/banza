# ADR-010 — BANZA Canonical JSON (BCJ/1)

- **Status:** Accepted
- **Date:** 2026-08
- **Relates:** ADR-009 (versioning decision), ADR-027 (Model A signing), ADR-027 (domain separation), ADR-043 (Rust-first engines)
...
| **P7** | The canonicalization identifier `BCJ/1` is carried by artifacts that declare their canonicalization | Makes the transition from the prior behaviour explicit rather than silent (ADR-009) |
...
form, which ADR-009 explicitly determined must not happen, and no finding requires it.
...
trust rules — those remain ADR-027 (Model A) and ADR-027 (domain separation). It confers no authority over
