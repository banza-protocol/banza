...
- **Status:** Accepted
- **Date:** 2026-07
- **Relates:** ADR-038 (open protocol trust model)

## Context
Under the open trust model (ADR-038), BANZA's trust material is anchored in an **offline root**
...
- Reinforces the open trust model's discipline that private key material never exists in serving
  infrastructure (ADR-038): verification runs off signed metadata (`INV-OTE-*`), not off any runtime key.
