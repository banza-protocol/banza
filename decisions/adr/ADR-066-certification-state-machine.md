# ADR-066 — Closed Certification-State Machine

- **Status:** Accepted
- **Date:** 2026-07
- **Milestone:** M2.19D
- **Related:** ADR-064 (Conformance & Interoperability Certification model), ADR-065 (BANZA Technical
  Registry), ADR-037 (Rust-first engines), ADR-038 (open trust model — signed, dated, fail-closed
  revocation), ADR-061 (certification ≠ admission ≠ authorisation)

---

## Context

ADR-064 requires each `CertificationRecord` to carry a lifecycle — validity, expiration, renewal,
suspension, revocation and supersession — and ADR-064 D-064-06 fixes that it must be scoped, time-limited and
...
| **D-066-03** | **`REVOKED` is terminal; no resurrection.** From `REVOKED` there is no transition back to `CERTIFIED`. A revoked implementation may only become certified again through an entirely **new** record with fresh evidence — never by reinstating the revoked one (ADR-038 INV-OTE-006). |
| **D-066-04** | **Renewal is re-certification, never extension.** A validity window is never extended in place. "Renewal" is a new `CertificationRecord` produced by re-running the profile against the (possibly new) artifacts; the prior record moves to `SUPERSEDED` (or stays `EXPIRED`). This keeps every certificate an honest statement about the evidence that produced it (ADR-064 D-064-01/03). |
| **D-066-05** | **Fail-closed everywhere.** `NOT_CERTIFIED`, `EXPIRED`, `SUSPENDED`, `REVOKED` and `SUPERSEDED` all read as **not a valid certification**. Only `CERTIFIED`, within scope and window and with reproducible evidence, reads as valid. An expired or unreadable record is never treated as certified (ADR-064 D-064-06, ADR-065 D-065-04). |
| **D-066-06** | **Rust decides; no override.** All transitions are computed by the Rust certification engine from evidence, signatures and the clock. No human and no model (local Qwen) may effect, widen or reverse a transition; no configuration or flag turns `NOT_CERTIFIED`/`SUSPENDED`/`REVOKED` into `CERTIFIED` (ADR-037, ADR-038 INV-OTE-008). Suspension and revocation are signed and dated; the engine validates the signature before applying them. |
| **D-066-07** | **No status propagation.** A record's state governs only L2 certification. `CERTIFIED` is not admission and not authorisation; a state transition on L2 never triggers, implies or announces a determination on L3 (scheme admission) or with the regulator (ADR-061 D-061-06). |
...
**Untouched.** No financial invariant; ADR-037 (Rust decides), ADR-038 (signed/dated/fail-closed revocation)
and ADR-061 (no propagation) all stand.

## References

- ADR-064 (certification model), ADR-065 (Technical Registry)
- `engines/banza-conformance` / the M2.19D certification engine — owns the transition table
