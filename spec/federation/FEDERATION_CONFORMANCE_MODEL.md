...
**Authority:** ADR-025, ADR-031, ADR-025, FEDERATION_INVARIANTS.md, FEDERATION_CONTRACT_SURFACE.md, FEDERATION_PROTOCOL_FLOW.md
...
Evaluation (ADR-025) — it is never a status conferred on the operator.
...
Because the Open Trust Evaluation is per-capability (ADR-025 check 7), a non-blocking failure fails closed
...
### 3.1 Trust Invariants (ADR-025, ADR-031, ADR-025)
...
Evaluation (ADR-025) over the operator's published material. The rules below state, for the reference
...
Because the Open Trust Evaluation is per-capability (ADR-025 check 7, INV-FEDEVAL-007), a failure in a
