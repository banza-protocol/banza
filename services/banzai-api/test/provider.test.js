...
// ── local_qwen (ADR-044): internal llama.cpp, keyless, on-host ────────────────
...
test("local_qwen uses professional-but-bounded defaults (max_tokens 384, timeout 60s); hosted unchanged; env overrides", () => {
  ...
  assert.equal(l.maxTokens, 384, "local default output is a professional 384 (ADR-047)");
...
// ADR-045 FIX-1/2: the SHIPPED compose injects an EMPTY string for these vars (so it does
...
test("local_qwen keeps its default under the shipped empty-string env (deploy path)", () => {
  ...
  assert.equal(l.maxTokens, 384, "empty LLM_MAX_TOKENS must fall back to the local default 384 (ADR-047)");
...
test("warmup() waits for /health to be ready, then primes with a 1-token request; not counted", async () => {
  ...
  // ADR-045 FIX-3: first poll /health at the server root, then prime the model.
...
// ── ADR-046: reasoning disabled for local (Qwen3) + prefix warm-up ────────────
