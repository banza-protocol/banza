...
const SEQ_B = ["Abre a ADR-005.", "Resume.", "Explica a motivação.", "Mostra as consequências.", "Compara com a ADR-006."];
...
const EXPECTED_TASK = {
  ...
  "Abre a ADR-005.": "document_lookup",
  "Explica a motivação.": "explanation",
  "Mostra as consequências.": "impact",
  "Compara com a ADR-006.": "comparison",
...
test("DFN-8 explicit subject/document in the current turn overrides prior context", () => {
  ...
  const r = routeCtx("Abre a ADR-006.", prior);
  assert.ok(/ADR-006/i.test(r.resolved_query || "Abre a ADR-006."), "explicit ADR-006 preserved over prior federation context");
  assert.equal(taskOf("Abre a ADR-006.", "ADR-006"), "document_lookup", "explicit document → lookup");
