...
test("(m2.18b2) candidate generation proposes REAL canonical documents (Part 16)", () => {
  const cands = generateCandidates("explica a ADR-002");
  assert.ok(Array.isArray(cands) && cands.length > 0, "ADR-002 must yield candidates");
  ...
  assert.ok(cands.some((c) => c.id === "ADR-002"), "ADR-002 is among the candidates");
});
