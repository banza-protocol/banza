...
test("isPublicSource — canonical public docs stay public", () => {
  assert.equal(isPublicSource({ id: "ADR-002", path: "decisions/adr/ADR-002-ecosystem-naming-inversion.md", category: "decision" }), true);
...
test("normalizeBanzaiAnswer — drops CLAUDE.md, keeps the ADR (the incident)", () => {
  const sources = [
    { id: "ADR-002", title: "ADR-002 — Ecosystem Naming Inversion", path: "decisions/adr/ADR-002-ecosystem-naming-inversion.md", category: "decision" },
  ...
  assert.ok(paths.includes("decisions/adr/ADR-002-ecosystem-naming-inversion.md"), "ADR-002 must be kept");
