...
test("(§8) the claim verifier classifies the five categories and enforces the rules", () => {
  const doc = buildFactualPackagePlanned("t", "explica a ADR-001 sobre a inversao de nomes", "ADR-001", "brief");
  ...
  const dead = verifyClaims(doc, { answer_markdown: "A ADR-001 inverte a nomenclatura.", claims: [{ claim: "inverte a nomenclatura", fact_ids: ["F1"] }], cited_source_ids: ["ADR-030"] });
  assert.equal(dead.ok, false);
  assert.deepEqual(dead.dead_citations, ["ADR-030"]);
...
test("(§8) a DERIVED metric claim must expose its full derivation, backed by an operational package", () => {
  ...
  const doc = buildFactualPackagePlanned("t", "explica a ADR-012", "ADR-012", "brief");
...
test("(§9) an unsupported claim or a dead citation is never published (fallback)", async () => {
  ...
  const unsupported = JSON.stringify({ answer_markdown: "algo sem base.", claims: [{ claim: "algo sem base", fact_ids: [] }], cited_source_ids: ["ADR-001"], insufficient_evidence: false });
  const r1 = await runGroundedSynthesis("explica a ADR-001", { provider: scriptedProvider([unsupported]), entityId: "ADR-001" });
  ...
  const dead = JSON.stringify({ answer_markdown: "Na verdade a ADR-030 trata disto.", claims: [{ claim: "trata disto", fact_ids: ["F1"] }], cited_source_ids: ["ADR-030"], insufficient_evidence: false });
  const r2 = await runGroundedSynthesis("explica a ADR-001", { provider: scriptedProvider([dead]), entityId: "ADR-001" });
...
test("(§7) a published grounded trunk answer surfaces the compact factual_package in meta", async () => {
  const pkg = buildFactualPackagePlanned("t", "por que a ADR-012 usa dupla entrada", "ADR-012", "brief");
  const trunkStub = async () => ({
    status: "grounded",
    answer_markdown: "A ADR-012 estabelece a dupla entrada no ledger do protocolo.",
    cited_source_ids: ["ADR-012"],
    package: pkg,
  ...
  const { result, meta } = await pipeline.answer("explica por que a ADR-012 usa dupla entrada no ledger");
