...
test("(m2.18b6 §14) the planned package carries the contract versions + a package checksum for cache keying", () => {
  const p = buildFactualPackagePlanned("t", "explica a ADR-012 sobre dupla entrada", "ADR-012", "");
  const v = contractVersions();
  ...
  const q = buildFactualPackagePlanned("t", "explica a ADR-012 sobre dupla entrada", "ADR-012", "");
