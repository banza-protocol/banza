// Fase D — durable validation-receipt E2E (ADR-076). Deterministic, no network.
//
...
async function main() {
  console.log("== receipts-e2e (ADR-076 durable receipts, live PG) ==");
  ...
  await step("the RUNNING row is persisted with lifecycle=RUNNING + heartbeat (ADR-076 correction 2)", async () => {
  ...
  await step("finalizeJourney seals the aggregate and freezes the execution", async () => {
    ...
    assert.equal(row.execution_lifecycle, "COMPLETED", "lifecycle advances to COMPLETED at finalize (ADR-076 §6)");
  ...
  // ── 3. DB-enforced append-only immutability (ADR-076 §D-076-08) ──
