...
for (const f of ["ADR-035", "ADR053", "ADR 053", "adr_0041", "adr-53", "RFC-0006", "RFC0006", "RFC 0006", "rfc 6"]) add(`explica o ${f}`, "id", { resolves: true });
...
for (const f of ["ADR-035", "ADR-001", "ADR-012", "ADR-036", "RFC-0006"]) add(`resume o ${f}`, "id", { resolves: true });
...
for (const q of ["qual estado ADR053 e porque foi aceite", "compara a ADR-035 com a ADR-036", "explica fedaracao e compara com revogacao", "qual licenca e o que ela permite", "o que muda para o operdor", "compara a adr 002 com a adr 001", "explica o manfesto e da um exemplo", "qual o estado da adr054 e o seu impacto", "governanca e confianca qual a relacao", "qual rfc fala de pagamento offline e porque"]) add(q, "mixed-intent", {});
