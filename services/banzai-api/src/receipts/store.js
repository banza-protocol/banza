// Durable, append-only validation receipt store (ADR-036). DB operations over the
...
//    sealed — ADR-036), so there is no mid-flight UPDATE to fight the freeze trigger.
...
// ── ADR-036 read-only telemetry: journey / per-step DURATION metrics over persisted executions ──────
//
...
//    a verification action, not a fresh journey) and their count is reported for transparency (ADR-036 §1);
...
// call sites can ever disagree on the method (ADR-036 §2):
