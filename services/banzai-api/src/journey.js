// BanzAI operator-guidance backend glue (ADR-049; reframed by ADR-076 §D-076-01/02). RUST_WRAPPER_ONLY.
//
// REGRA (ADR-076 §D-076-01): Modelo A orienta o percurso; Modelo B avalia — existe uma única
...
// The legacy flat verdict fields. Under ADR-076 §D-076-02 only their PRESENCE is read (as "visited");
...
export function deriveJourney(journeyContext, currentStep) {
  ...
  try {
    ...
    return {
      ...
      // ADR-076 — the answer to "o que faço agora?", composed in Rust from the SAME next orientation
      ...
      // There is exactly one authority of technical validation state, and it is Model B (ADR-076
