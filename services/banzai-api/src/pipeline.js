...
// M2.19G.5C (ADR-073, D-073-05) — the post-synthesis authority-validator POLICY version, bound into the
...
// the terminal AFTER the mandatory ADR-073 + Inc.4 validation, carrying the finished /ask envelope). There
...
// M2.19G.5C (ADR-073, check 22) — the deterministic contradiction backstop for the grounded publish gate.
...
export function progressDisposition(result, meta) {
  ...
  if (m.terminal_kind === "safety_refusal" || m.intent === "safety_refusal" || m.fallback_reason === "safety_refusal") {
    ...
    // A DEGRADED / emergency grounding: the model answer was NOT published (ADR-073 post-validation reject,
...
export function createPipeline(provider, env = process.env, { nowFn = Date.now, inferenceRun, runGroundedSynthesisFn, liveArtifactTool = null, telemetryTool = null, receiptsTool = null } = {}) {
  ...
  // M2.19G.5C (ADR-073, D-073-06) — the post-synthesis authority validator is MANDATORY by default. The
  ...
  async function answer(question, { mode: requestedMode, contextQuestions, conversationContext, journeyStep, documentId, journeyNextActionSentence, signal, requestId, onProgress } = {}) {
    ...
    // ANY of them evicts stale entries. Caching remains downstream of the ADR-073 gate (validated-only).
    ...
    // Tier 0c — OPERATIONAL METRIC (ADR-078; Rust-decided, 0 model calls). A duration/metric/live-state
    ...
    // estado da ADR-053?"). A structured document_id from the UI always means "explain the record", so the
    ...
    // SPR-3 — a cache hit serves a FINAL_VALIDATED answer (cache writes sit downstream of the ADR-073 gate),
    ...
    if (tp.status === "grounded" && tp.answer_markdown) {
      // ── M2.19G.5C (ADR-073) — the MANDATORY post-synthesis authority validator, on the EXACT bytes that
      ...
      // CLAIM_VERIFICATION_STARTED — the MANDATORY ADR-073 post-synthesis authority/claim gate begins over
  ...
  function usage() {
    return {
      ...
      // M2.19G.5C (ADR-073) — safe post-synthesis-validator telemetry (counts/enums only, never content).
