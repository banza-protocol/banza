...
// inference happens off-host. `local_qwen` (ADR-044) calls a LOCAL, internal-only
...
// `local_qwen` (ADR-044) is an internal-only llama.cpp OpenAI-compatible endpoint on
...
// network). ADR-044: local/external classification is driven by the resolved
...
export function readLlmConfig(name, env = process.env) {
  ...
  return {
    ...
    onHost: isOnHost(apiBase), // resolved-destination classification (ADR-044)
    // ADR-045/047 latency tuning: local inference defaults to a professional-but-bounded
    // output (384; was 256 in ADR-045 — with reasoning disabled per ADR-046 the budget goes
...
export function buildChatRequest(name, question, grounded, cfg, { maxTokens, mode } = {}) {
  ...
  // ADR-046: Rust owns the policy (disable_reasoning); this glue maps it to the local
...
function createRealProvider(name, env, fetchImpl) {
  // Classify by the RESOLVED destination host, not the provider name (ADR-044): a
  ...
  return {
    ...
    // ADR-046: null until warm-up runs, then true (primed) / false (never got ready).
    get warmupState() {
      return warmedState;
    },
    // ADR-045: best-effort warm-up for local inference. On a cold full-stack boot the
    // llama.cpp model is NOT in banzai-api's depends_on and takes ~90s to load, so a
    // one-shot ping would fire against an unready endpoint and never retry (ADR-045
    ...
    async warmup({ retries = 24, delayMs = 5000, readyTimeoutMs = 4000 } = {}) {
      ...
      try {
        // ADR-046: prime the REAL compact system-prompt prefix (from Rust) so the first
    ...
    async answer(question, opts = {}) {
      ...
      return {
        ...
        // Per-answer proof that this specific answer called the model (ADR-044 telemetry).
        model_called: true,
