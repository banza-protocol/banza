...
const KNOWN_WORDS_JSON: &str = include_str!("known-words.json");
...
/// Every word the declared vocabulary contains — generated by `tools/gen-banzai-known-words.mjs` from
/// all entry keywords plus the declared domain aliases, so this cannot drift from what the engine
/// actually knows.
fn known_words() -> &'static [String] {
    static W: OnceLock<Vec<String>> = OnceLock::new();
    &W.get_or_init(|| {
        serde_json::from_str::<KnownWords>(KNOWN_WORDS_JSON)
            .expect("known-words.json")
            .words
    })[..]
}
...
fn registered_surface_forms() -> &'static std::collections::HashSet<String> {
    ...
    V.get_or_init(|| {
        ...
        // The WHOLE declared vocabulary, not the routing subset.
        //
        // The loop above walks `entries()`, which is the entries INDEX — 28 lexically routable entries
        // out of 215 — so almost everything this engine knows was invisible to known-word protection.
        // `public` was not in those 28 and `publica` was, so recovery rewrote "What is a public key?"
        // to "what is a publica key" at high confidence, before any router saw it. The same happened to
        // contrato→controlo, schema→scheme and transfer→transfere: four correct words turned into
        // different words, each then unanswerable.
        //
        // Routing eligibility and known-word protection are different questions, and this is the second
        // one, generated from every entry keyword and every declared domain alias.
