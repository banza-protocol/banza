...
//! ("qual o estado da ADR-035, por que foi aceite e qual o impacto para operadores?") preserves EVERY
...
mod tests {
    ...
    fn mixed_request_preserves_every_part() {
        ...
        let p = plan(
            "qual é o estado da ADR-035, por que foi aceite e qual o impacto para operadores?",
            "ADR-035",
        );
        ...
        assert!(p.entities.contains(&"ADR-035".to_string()));
    ...
    fn example_request_becomes_explain_with_example() {
        let p = plan("explica a revogação com um exemplo", "ADR-025");
    ...
    fn plain_explanation_is_one_call_with_mandatory_citations() {
        let p = plan("o que é a federação?", "ADR-025");
        assert_eq!(p.answer_type, AnswerType::GroundedSynthesis);
        assert_eq!(p.expected_model_calls, 1);
        assert_eq!(p.citation_requirements, "mandatory");
    ...
    fn plan_is_deterministic() {
        let a = plan("qual o impacto da ADR-035 para operadores?", "ADR-035");
        let b = plan("qual o impacto da ADR-035 para operadores?", "ADR-035");
