...
mod tests {
    ...
    fn procedure_answered_with_an_adr_list_is_incomplete() {
        ...
        let v = check_completion(
            &o,
            "Para federar um operador, deve-se seguir o modelo de confiança aberto sem CA (ADR-025) ou a avaliação de confiança sem certificados (ADR-025).",
            &src(2),
            2,
            true,
        );
