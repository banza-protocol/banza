//! Rust-first policy enforcement guard (ADR-037).
//!
...
/// (allowed by ADR-037), never engines themselves — so an engine marker inside a guard script
...
pub fn classify(rel_path: &str, content: &str, allow: &BTreeSet<String>) -> FileReport {
    ...
    let (verdict, note) = if wrapper_marked {
        (
