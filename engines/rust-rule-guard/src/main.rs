//! `rust-rule-guard` — enforce ADR-037 (Rust-first for official engines).
//!
//! Usage:
...
fn main() {
    ...
    if !quiet {
        println!(
            "rust-rule-guard (ADR-037) — scanned {} code files",
            report.scanned
        );
    ...
    if blocked.is_empty() {
        ...
        println!(
            "\n✗ FAIL — {} new non-Rust engine(s) blocked by ADR-037:",
            blocked.len()
        );
        for f in &blocked {
            println!("  ✗ {}", f.path);
