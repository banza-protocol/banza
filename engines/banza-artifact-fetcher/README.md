# banza-artifact-fetcher (ADR-037, ADR-068 §4.7 / §18–§20; M2.19G.1)
...
journey (ADR-068). The no-network protocol engines (`banza-operator-manifest`, `banza-trust`,
...
> Operational rule (ADR-068 §4): *the operator publishes; BanzAI obtains (this fetcher); Rust
> verifies; the receipt fixes the result; the Technical Registry publishes the verifiable state.*

Per ADR-037 this is an **official engine → it MUST be Rust**. It ships as **both**:
...
| `src/audit.rs`     | `request_id`, RFC3339 timestamp, SHA-256, and the one-line JSON audit record (ADR-068 §4.8). |
...
the **closed Technical Registry** (ADR-065) — never from a user-supplied URL (ADR-068 §4.4).
...
## SSRF policy (ADR-068 §19) — every rule a distinct `reason_code`
...
services:
  banza-fetcher:            # ADR-068 §4.7 secure artifact fetcher; the ONLY egress to operator endpoints
...
- Verification/transport only. It NEVER decides a certification verdict, signs, issues certificates,
  or moves funds. It obtains bytes; the Rust decision engines judge them (ADR-068 §Decision).
