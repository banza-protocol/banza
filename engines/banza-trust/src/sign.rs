//! banza-trust TEST-ONLY signer + fixtures + ceremony simulator (ADR-038; M2.4).
//!
...
impl TestKeypair {
    ...
    /// Sign a document over the ADR-025 canonical form (all fields except `sign_field`), returning the
...
/// active model (ADR-025). Exposed so the federation OTE battery can mint an authentically-signed BRL.
