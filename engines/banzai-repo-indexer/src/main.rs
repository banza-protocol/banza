//! banzai-repo-indexer (M2.13B PR2, ADR-038/ADR-036).
//!
//! A deterministic, dependency-light Rust indexer that gives the BanzAI agent SAFE, CURRENT,
//! CITABLE knowledge of the whole BANZA monorepo — which since M2.19G.6 (ADR-036) includes the
...
//! network, and NEVER emits secret material. TS/JS remains UI/glue only (ADR-038).
//!
...
// M2.19G.6 (ADR-036): the separate banza-protocol/banzai repo was consolidated into this monorepo and
...
fn path_excluded(rel: &str) -> Option<&'static str> {
    ...
    // repository (ADR-036). Retained in the repo for governance history, but NEVER re-indexed into active
...
fn chunk_is_contaminated(text: &str) -> bool {
    ...
    let brands = [
        ...
        "ekwanza",
        " emis ",
        // M2.19C: Banzami (designated L3 scheme operator, ADR-004/060) is groundable now; only real
...
fn main() {
    ...
    // M2.19G.6 (ADR-036): monorepo-only CLI — <banza_repo> <banza_commit> <out_dir> [indexed_at].
    ...
    // M2.19G.6 (ADR-036): no sibling repo is indexed — the former banza-protocol/banzai content is
    ...
    let manifest = serde_json::json!({
        ...
        // M2.19G.6 (ADR-036): the monorepo is the sole indexed source. The consolidated BanzAI runtime
...
mod tests {
    ...
    fn does_not_exclude_real_sources() {
        for p in [
            "LICENSE",
            "README.md",
            "decisions/adr/ADR-035-operator-zero-the-read-only-reference-implementation.md",
