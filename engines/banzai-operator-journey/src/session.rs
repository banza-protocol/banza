//! Operator-guidance navigation model (ADR-036/02, Fase B).
//!
//! REGRA (ADR-036)
//! -------------------------
...
/// the orientation activity was visited/finished, never a technical approval (ADR-036).
...
/// The canonical six Model B per-step states (ADR-036). Model A only ever REFERENCES these
...
/// A typed, opaque reference to a Model B execution/step (ADR-036). Ids only — never a body,
...
/// Normalise a referenced Model B state to the canonical six (ADR-036) or empty. An
...
/// under ADR-036 it is deliberately NOT interpreted as one: its mere PRESENCE is read as
...
/// technical — no Model B reference, state, artifact or verdict influences it (ADR-036).
...
pub fn evaluate_session(state: &Value) -> Value {
    ...
    json!({
        // ADR-036 markers: this is the guidance layer, and Model B is the state authority.
...
/// (ADR-038), so Rust publishes them and the vocabulary contract guard proves the UI matches. There
/// are no `blocker_reasons`/`evidence_items` here: the guidance layer has no verdict to block on and
/// no evidence to produce (ADR-036).
