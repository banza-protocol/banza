//! banzai-operator-journey (ADR-036; reframed by ADR-036/02) — the Rust state machine for the
...
//! `/ask` — is computed here (ADR-038; rules 15/16). Deterministic; no LLM, no network, no persistence.
//!
//! GUIDANCE ONLY (ADR-036/02): this layer orients the percurso; it does NOT evaluate. Its
...
// The allowed per-activity NAVIGATION status values (ADR-036) are owned by
...
/// The legacy flat verdict field name for a step. Under ADR-036 only its PRESENCE is read
...
// replaced it, are both gone (ADR-036): this layer no longer computes any verdict or score.
...
// ── Upload JSON scan (M2.9C, ADR-036) ────────────────────────────────────────
