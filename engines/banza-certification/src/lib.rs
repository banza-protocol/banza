//! banza-certification (ADR-038 R4; ADR-032/065/066) — the single deterministic Rust authority for
...
//! machine (ADR-032). Every decision is:
//!   deterministic · idempotent · fail-closed · reproducible · hash-bound · profile-bound · scope-bound ·
//!   environment-bound — with **no Qwen, no external model, no human FAIL→PASS** (ADR-038/038/061).
//!
...
//! (ADR-005). This crate holds no funds, moves no value and admits no participant.
...
// ── Closed enums (ADR-032 / ADR-032) ─────────────────────────────────────────

/// The closed certification-state machine (ADR-032). Any value outside this enum is a bug; an
...
impl CertificationStatus {
    /// Fail-closed reading (ADR-032, ADR-032): only `Certified` is a valid certification.
    pub fn is_valid(self) -> bool {
        matches!(self, CertificationStatus::Certified)
    }
}

/// Lifecycle events that drive transitions (ADR-032).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
...
/// Closed set of certification reason codes (ADR-032 / ADR-005). `Ok*` = pass; every `Fail*` is a precise,
...
/// The public, versioned yardstick (ADR-032).
...
/// The subject of certification (ADR-032): an implementation, identified by artifact hash.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// schema conformance (ADR-032; the L2 boundary is conformance AND interoperability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Signed, dated suspension (ADR-032).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Signed, dated revocation (ADR-032, terminal).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Renewal is re-certification (a NEW record), never an in-place extension (ADR-032).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
// ── The deterministic certification pipeline (ADR-032, fail-closed) ──
...
/// The 20-step decision (ADR-032): validate inputs → schemas → canonicalisation → hashes → signatures →
...
fn decide(i: &CertifyInput) -> (CertificationStatus, ReasonCode, Vec<String>, Vec<String>) {
    ...
    // revocation short-circuits everything (fail-closed, ADR-032)
    if i.trust.revoked {
    ...
    // conformance AND interoperability must both pass (never certify on schemas alone, ADR-032)
...
/// Verify a record's `record_hash` reproduces (ADR-032). Fail-closed: any mismatch → false.
...
/// Temporal + revocation evaluation of a record's EFFECTIVE status at `now` (ADR-032). Fail-closed:
...
/// A compact registry entry view for a record (ADR-033) — what the public Technical Registry publishes.
