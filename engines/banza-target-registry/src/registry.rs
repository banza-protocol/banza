//! The closed Technical Registry (ADR-068 §4.6/§14): the ONLY source of validation targets.
//!
...
/// The canonical reference operator's public origin (ADR-067). Not a hard protocol dependency — it is
...
impl Registry {
    ...
    /// every eligibility rule (ADR-068 §14). Returns the resolved target or a typed reason.
    pub fn resolve(
        &self,
        operator_id: &str,
...
/// (`operator-zero-ref-impl`). No fictional operators (ADR-068 §4.9).
