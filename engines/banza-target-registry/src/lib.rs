//! banza-target-registry (M2.19G.1, ADR-034) — the CLOSED Technical Registry domain model plus the
...
//!
//! Boundaries (ADR-034 §4.10, ADR-005): resolution proves eligibility only. Presence in the registry
...
/// steps. The single Rust-decided source for what a profile requires (ADR-030 levels, ADR-030).
...
// ---------------------------------------------------------------------------------------------------
// Unit tests (ADR-034 §38 — model + registry). Resolution/eligibility logic exercised over test-only
...
mod tests {
    ...
    fn l0_readiness_excludes_not_applicable_l2_l3_steps() {
        // OZ-3 (ADR-030): an L0 implementation whose REQUIRED steps are all VERIFIED is READY even when
