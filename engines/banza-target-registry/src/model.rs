//! Domain model for the closed Technical Registry (ADR-034 §13).
//!
...
//! entity in the abstract (ADR-034 §4.2/§4.3). Presence of a record NEVER implies admission,
//! authorisation, or the ability to move funds (ADR-034 §4.10, ADR-005).
...
/// The published artifact endpoint map — the 14 canonical paths of an implementation (ADR-034 §13).
...
impl Endpoints {
    /// The canonical published paths of the reference implementation (ADR-035 served surface + the
    /// four endpoints ADR-034 requires: discovery, capabilities, signed-metadata, federation-manifest).
    /// The operator (discovery) manifest and the signed protocol metadata are published at their canonical
    /// `.well-known/banza/` discovery routes per ADR-029 (RFC-0005 / ADR-031); the remaining demo artefacts
    ...
    pub fn reference() -> Self {
        Endpoints {
            ...
            // endpoints the implementation actually serves (ADR-034 §4.6/§22), so this points at the
...
/// An implementation record — the technical system actually evaluated (ADR-034 §4.2).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ImplementationRecord {
    ...
    /// Empty string = origin-less (an ineligible target — ADR-034 Consequences).
...
/// only — never admission, authorisation, or the ability to move funds (ADR-034 §4.10).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Typed ineligibility / resolution-failure reasons (ADR-034 §14). Serialized as snake_case so the
