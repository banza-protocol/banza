...
//!
//! ADR-068: **Rust decides every verdict; Qwen only explains; TypeScript never decides.** The decision
...
//! Record and **never** `CERTIFIED` (ADR-068 §21 step 9, §4.10).
...
/// Validate a fetched discovery document against the resolved target (ADR-068 §21 step 1). Checks the
...
/// Applicability of a step to a profile — a dimension ORTHOGONAL to the step's state (ADR-077).
...
/// Applicability of one technical step to a declared conformance profile (ADR-021 levels, ADR-077
...
/// Aggregate the technical step verdicts into a Certification **Readiness** record (ADR-068 §21 step 9,
/// ADR-077 profile applicability). `step_verdicts` is an array of `{ "step": "...", "status": "..." }`.
