//! Native tests for the ADR-044 language-generation control logic:
...
fn system_prompt_is_compact_but_keeps_every_invariant() {
    // ADR-045 latency tuning: the system prompt must stay well under the pre-tuning size
    ...
    for invariant in [
        ...
        "guarda chaves privadas",                // key-custody boundary (ADR-045 FIX-4)
...
fn validator_blocks_infra_key_custody_claims_but_allows_the_grounded_answer() {
    // ADR-045 FIX-4: the compact prompt kept the key-custody clause; the validator now
...
fn prompt_builder_wraps_untrusted_data_and_defends_injection() {
    ...
    // ADR-046: the prompt contract carries the reasoning policy (BanzAI never reasons).
    assert_eq!(
        v["disable_reasoning"].as_bool(),
        Some(true),
        "prompt contract must declare disable_reasoning=true (ADR-046)"
    );
}
