...
| `validate-schema.sh` | `postgres/init/001_schema.sql` applies on ephemeral `pgvector/pgvector:pg16`: 23 tables (incl. `banzai_answer_cache` with hnsw embedding index, the onboarding tables, and the ADR-042 validation-journey receipt store with append-only triggers), segregated roles, pgvector active, `protocol_state.phase = pre-production`, `operators` empty. |
