...
-- points to. BANZA issues nothing to operators (ADR-038): no certificate, no granted
...
-- entity-level status (ADR-038/040). An entry names the revoked material via revoked_ref
...
INSERT INTO protocol_state(k,v) VALUES
  ...
  ('note', '"Um PASS de conformance é evidência técnica verificável, não uma certificação nem um estatuto concedido. A BANZA não emite certificados a operadores (modelo de confiança aberto, ADR-038): a participação demonstra-se por evidência publicada, não é concedida por uma autoridade central. Nenhum operador publicou evidência de produção neste ambiente."');

-- ─────────────────────────── Operator onboarding (M2.19G.3, ADR-069) ──────────────
...
-- the closed Technical Registry). Onboarding is a hosted service, never a protocol rule (ADR-069).
...
-- ─────────── Durable validation-journey receipt store (ADR-076 §D-076-08) ────────────
...
CREATE TABLE validation_executions (
  ...
  overall_status text NOT NULL DEFAULT 'NOT_EVALUATED',       -- 6-state model (ADR-076 §D-076-04)
  ...
  -- Operational state (mutable while RUNNING; ADR-076 §D-076-08 crash-recovery/locking/idempotency).
  ...
  -- Telemetry precision (BZO-9, ADR-078): execution-kind classification + monotonic µs duration.
...
-- Immutability-preserving execution-kind attestation (BZO-9, ADR-078): records the kind of an ALREADY-FROZEN
...
-- DB-enforced append-only immutability for the sealed artefacts (ADR-076 §D-076-08): no UPDATE, no DELETE.
...
-- BanzAI owns the validation-journey receipt store (ADR-076). Live orchestration rows advance then
