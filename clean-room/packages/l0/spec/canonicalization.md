...
- **Authority:** [ADR-011](../decisions/adr/ADR-011-banza-canonical-json-bcj-1.md); versioning per [ADR-008](../decisions/adr/ADR-008-normative-authority-and-versioning.md)
...
3. Resolve the verification key by `key_id` through the trust path in force (ADR-025 Model A: the Trust
   Root signs only the Key Manifest; delegated keys sign domain artifacts per ADR-025).
...
`BCJ/1` is versioned independently of the protocol version (ADR-008). A change to any rule in §2–§6 that
